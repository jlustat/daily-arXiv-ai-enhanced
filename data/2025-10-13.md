...
Motivation: To isolate causal effects of text properties on audience attitudes/behaviors while controlling for latent confounding factors that traditional LLM methods fail to address properly.

Method: Proposes a new experimental design that handles latent confounding without inducing overlap bias, applied to study the persuasiveness of expressing humility in political communication.

Result: LLM-based methods performed worse than simple bag-of-words models when tested with real text and experimental outcomes; successfully isolated causal effect of humility on persuasiveness.

Conclusion: The new experimental design provides unbiased treatment effect estimation and offers insights for social media platforms, policymakers, and social scientists on communication effects.
...
<details>
  <summary>Details</summary>
Motivation: Bayesian mixture models often produce overlapping or redundant components due to independently specified priors on cluster locations, which hinders interpretability. Existing repulsive priors face computational and theoretical challenges.

Method: Assign a projection Determinantal Point Process (DPP) prior to component locations, which induces strong repulsion and allows exact sampling. Derive closed-form posterior and predictive distributions, enabling two efficient inference algorithms: conditional Gibbs sampler and the first fully implementable marginal sampler for DPP-based mixtures.

Result: Superior mixing and clustering performance compared to alternatives in misspecified settings. Application to event-related potential functional data uncovers interpretable neuro-cognitive subgroups. Provides strong frequentist guarantees including posterior consistency for density estimation, elimination of redundant components, and contraction of the mixing measure.

Conclusion: Projection DPP mixtures offer a theoretically sound and practically effective solution for Bayesian clustering, enabling parsimonious and interpretable posterior clustering with strong theoretical guarantees.
...
<details>
  <summary>Details</summary>
Motivation: To address the complexity of handling numerous possible treatment hierarchy questions in network meta-analysis with many treatments, eliminating the need for pre-selecting specific questions and enabling extraction of all meaningful insights supported by data.

Method: Developed efficient and scalable algorithms that systematically generate a comprehensive catalog of highly credible treatment hierarchy questions (with empirical probabilities exceeding a chosen threshold), with an additional algorithm to trim redundant insights. Covers six types of binary hierarchy questions including pairwise comparisons and ranking probabilities.

Result: Implemented methods in an R package and demonstrated application using real NMA datasets on diabetes and depression interventions. The approach is scalable and applicable to any decision problem involving three or more treatment options.

Conclusion: The proposed algorithmic approach provides a systematic way to extract meaningful treatment hierarchy insights from network meta-analysis data without pre-specification, making it more efficient and comprehensive than previous methods that required manual question selection.
...
<details>
  <summary>Details</summary>
Motivation: Traditional poverty measurement based only on income/consumption is insufficient; there's growing need for multidimensional approaches that capture poverty more reasonably, especially for small areas where sample sizes are limited.

Method: Uses multivariate hierarchical model implemented via Bayesian method to estimate multidimensional poverty measures and dimension-specific contributions for small areas.

Result: Demonstrates methodology using specially designed survey data for multidimensional poverty mapping, successfully addressing the estimation of relative dimension contributions.

Conclusion: Adds new direction to poverty mapping literature by solving the previously ignored problem of estimating dimension-specific contributions in multidimensional poverty mapping for small areas.
...
<details>
  <summary>Details</summary>
Motivation: Stepped wedge designs face substantial inference challenges including confounding of intervention effects with secular time trends, cluster heterogeneity, complex correlation structures, baseline covariate imbalances, and small numbers of clusters.

Method: Proposed unified semiparametric framework with nonstandard efficiency theory that accommodates correlated observations, varying cluster-period sizes, and weakly dependent treatment assignments. Includes permutation-based standard error estimation with leave-one-out correction for finite-sample inference.

Result: The estimator is consistent and asymptotically normal even under misspecified covariance structure and control cluster-period means, and efficient when both are correctly specified. Simulations and application demonstrate robustness and efficiency relative to standard approaches.

Conclusion: The proposed framework provides a robust and efficient solution for estimating intervention effects in stepped wedge designs, addressing key methodological challenges through semiparametric estimation and design-based inference methods.
...
<details>
  <summary>Details</summary>
Motivation: Existing algorithms for finding minimal collapsible sets in general graphs are computationally demanding, limiting practical application in high-dimensional settings.

Method: Developed the Close Minimal Separator Absorption (CMSA) algorithm that constructs minimal collapsible sets using only local separator searches, significantly reducing computational costs.

Result: Simulations demonstrate substantial efficiency gains compared to existing methods, making collapsibility analysis practical for high-dimensional problems.

Conclusion: The CMSA algorithm provides an efficient and practical approach for dimension reduction in contingency tables and graphical models by leveraging the relationship between collapsibility and minimal separators.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitation of traditional boxplots in detecting bimodality and multimodality, providing a tool that can identify and visualize meaningful subgroups in univariate data.

Method: A univariate clustering method is developed that ensures contiguous clusters (no cluster members inside another cluster) and each cluster contains at least a given number of unique members. The resulting bixplot displays individual data values and can incorporate external variables through color gradations.

Result: The bixplot successfully facilitates identification and interpretation of potentially meaningful subgroups in data, drawing attention to isolated points. Implementations are available in Python and R with various options demonstrated on real datasets.

Conclusion: The bixplot provides an effective exploratory tool for detecting and visualizing bimodality and multimodality in univariate data, extending the capabilities of traditional boxplots while maintaining interpretability.
...
<details>
  <summary>Details</summary>
Motivation: To address the challenge of computing sensitivity of failure probability to system parameters, which is more difficult than computing probability itself due to large input variables, rare events, and black-box nonlinear responses.

Method: Proposes a theory and Monte Carlo strategy that expresses sensitivity via expectation of response gradient conditional on threshold, resolved using kernel smoothing. Uses response values and gradients with respect to sensitivity parameters.

Result: The method provides sensitivity estimates for all response thresholds in a single Monte Carlo run, demonstrated through examples with different types of sensitivity parameters.

Conclusion: The approach enables embedding sensitivity calculations with reliability analysis in the same Monte Carlo run, particularly useful as response gradients become more available.
...
<details>
  <summary>Details</summary>
Motivation: Parameter estimation from complex survey samples typically focuses on global parameters, but multi-level models require adjustments for both local (group-level random effects) and global parameters to account for the sampling design.

Method: The authors evaluate limitations of survey-weighted pseudo-posterior and an existing automated post-processing method, then propose modifications to the automated process for multi-level models.

Result: The proposed modifications demonstrate improvements for multi-level models through a simulation study and application to the National Survey on Drug Use and Health data.

Conclusion: The updated method provides better incorporation of complex survey design in Bayesian multi-level models, with reproduction examples and an updated R package available.
...
<details>
  <summary>Details</summary>
Motivation: Address researchers' concerns about nonparametric methods losing power in small samples or overfitting when simpler models suffice, by showing these concerns are unfounded when nonparametric models are strongly shrunk towards parametric submodels.

Method: Expand parametric models with nonparametric components heavily shrunk toward zero, using Bayesian nonparametric models anchored to linear regression (including Gaussian processes regression and Bayesian additive regression trees variants).

Result: The models consistently identify correct parametric submodels when they hold and provide asymptotically efficient inference for regression coefficients. In simulations, "general BART" performs identically to correctly specified linear regression when parametric model holds, and substantially outperforms it when nonlinear effects are present.

Conclusion: Proposes "defensive model expansion" as a practical paradigm to safeguard against model misspecification, combining parametric efficiency with nonparametric flexibility through strong shrinkage mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: To address the limitations of traditional sampling methods like Furthest Point Sampling (FPS) that systematically under-sample equilibrium geometries, leading to poor predictive performance for relaxed molecular structures.

Method: Gradient Guided Furthest Point Sampling (GGFPS) extends FPS by incorporating molecular force norms to guide efficient sampling of molecular configurational spaces.

Result: GGFPS enables up to 2x training cost reduction without sacrificing accuracy, systematically lowers prediction errors for both equilibrium and strained structures in MD17 dataset, and decreases prediction error variances across all configuration spaces.

Conclusion: Gradient-aware sampling methods like GGFPS are promising training set selection tools, while naive use of FPS can lead to imbalanced training and inconsistent predictions.
...
TL;DR: A novel representer theorem for kernel methods in multivariate Hawkes processes, where triggering kernels are estimated via transformed kernels with dual coefficients fixed to unity, enabling efficient computation without costly optimization.


<details>
  <summary>Details</summary>
Motivation: To develop a computationally efficient method for estimating latent triggering kernels in multivariate Hawkes processes within an RKHS framework, addressing the computational challenges of conventional nonparametric approaches.

Method: Proposed a penalized least squares minimization approach where transformed kernels are defined via simultaneous integral equations, and optimal estimators are linear combinations of these transformed kernels with dual coefficients analytically fixed to unity.

Result: The method achieves competitive predictive accuracy on synthetic datasets while substantially improving computational efficiency compared to existing state-of-the-art kernel method-based estimators.

Conclusion: The novel representer theorem enables highly efficient estimation of triggering kernels in multivariate Hawkes processes by eliminating the need for costly dual coefficient optimization, making it suitable for large-scale data applications.
...
<details>
  <summary>Details</summary>
Motivation: Standard log-barrier mirror maps cause heavy-tailed dual distributions leading to ill-posed dynamics, and Gaussian priors perform poorly when matching heavy-tailed targets in generative modeling on convex domains.

Method: Proposes Mirror Flow Matching using regularized mirror maps to control dual tail behavior and guarantee finite moments, coupled with Student-t priors that align with heavy-tailed targets and stabilize training.

Result: Theoretical guarantees include spatial Lipschitzness and temporal regularity of velocity fields, Wasserstein convergence rates for flow matching with Student-t priors, and primal-space guarantees for constrained generation. Empirically outperforms baselines in synthetic convex-domain simulations and achieves competitive sample quality on real-world constrained generative tasks.

Conclusion: The proposed Mirror Flow Matching method effectively addresses fundamental challenges in generative modeling on convex domains through regularized mirror maps and Student-t priors, providing both theoretical guarantees and empirical improvements.
...
<details>
  <summary>Details</summary>
Motivation: To extend classical neural network approximation theorems beyond the traditional L^p-setting by proving density in Orlicz spaces, and to establish universal approximation uniformly with respect to weakly compact families of measures for practical neural network architectures.

Method: The authors prove that several classes of neural networks are dense in Orlicz spaces, which are more general function spaces than traditional L^p spaces. This is achieved through mathematical analysis of approximation properties.

Result: The paper establishes universal approximation properties uniformly with respect to weakly compact families of measures for feedforward neural networks with non-polynomial activation functions, deep narrow networks with ReLU activation functions, and functional input neural networks.

Conclusion: The results significantly extend classical universal approximation theorems by showing that various practical neural network architectures can approximate functions in Orlicz spaces, providing a broader theoretical foundation for neural network applications beyond traditional L^p settings.
...
<details>
  <summary>Details</summary>
Motivation: Traditional adversarial defenses are deterministic and don't account for uncertainty in attacks, while existing stochastic defenses lack statistical rigor and clear assumptions.

Method: Introduces a formal Bayesian framework that models adversarial uncertainty through a stochastic channel with explicit probabilistic assumptions, yielding proactive (training-time) and reactive (operational) defense strategies.

Result: The framework recovers several previous defenses as limiting cases and empirical validation shows benefits of explicitly modeling adversarial uncertainty.

Conclusion: The Bayesian approach provides a statistically rigorous foundation for adversarial defense that explicitly accounts for uncertainty, unifying and improving upon previous methods.
...
TL;DR: The paper introduces a causal autoregressive buffer that enables transformer-based probabilistic models to efficiently generate coherent joint distributions while maintaining flexible set-conditioning capabilities.


<details>
  <summary>Details</summary>
Motivation: Real-world applications require coherent joint distributions that capture dependencies between predictions, but existing approaches either sacrifice flexible set-conditioning (purely autoregressive models) or require expensive re-encoding at each step (standard set-based models).

Method: A causal autoregressive buffer that decouples context encoding from updating the conditioning set. The model processes context once and caches it, while a dynamic buffer captures target dependencies by allowing targets to attend to both cached context and previously buffered targets.

Result: The method matches predictive accuracy of strong baselines while delivering up to 20 times faster joint sampling across synthetic functions, EEG signals, cognitive models, and tabular data.

Conclusion: The approach combines the efficiency of autoregressive generative models with the representational power of set-based conditioning, making joint prediction practical for transformer-based probabilistic models.
...
<details>
  <summary>Details</summary>
Motivation: Real-world clinical problems often involve multimodal data with incomplete views and limited sample sizes, which pose significant challenges for machine learning algorithms.

Method: Proposes a Bayesian approach with dual formulation: (1) generative component for cross-view relationships with semi-supervised strategy, and (2) discriminative task-oriented component for downstream objectives.

Result: The approach successfully captures and disentangles complex interactions among biological, psychological, and sociodemographic modalities in clinical data.

Conclusion: The dual generative-discriminative formulation provides both general understanding and task-specific insights while enabling automatic imputation of missing views and robust inference across data sources.
...
<details>
  <summary>Details</summary>
Motivation: To develop a computationally efficient posterior sampling method that doesn't require likelihood evaluation and can capture complex posterior structures while providing theoretical guarantees.

Method: Flow matching approach that learns a dynamic, block-triangular velocity field in joint data-parameter space, creating deterministic transport maps. Uses monotone map constraints to enable conditional Brenier maps for fast credible set generation.

Result: Method is computationally lighter than GAN-based and diffusion-based approaches, captures complex posterior structures, and provides access to vector rank via reversible integration. Enables simultaneous generation of Bayesian credible sets with Monge-Kantorovich data depth contours.

Conclusion: The proposed flow matching sampler offers an efficient alternative to existing methods with theoretical consistency guarantees for posterior recovery and credible sets, while maintaining computational efficiency and handling complex posterior distributions.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM safety alignment techniques often make models over-cautious, causing them to refuse benign prompts. The goal is to enhance safety while reducing false refusals without compromising security.

Method: Train a lightweight external Energy-Based Model to assign high energy to undesirable states (false refusal or jailbreak) and low energy to desirable states (helpful response or safe reject). During inference, use the energy gradient to dynamically steer LLM hidden states to low energy regions in real-time without modifying model weights.

Result: Extensive experiments show EDS substantially lowers false refusal rates while maintaining baseline safety performance. For example, it raised compliance on ORB-H benchmark from 57.3% to 82.6% without compromising safety.

Conclusion: EDS presents an effective paradigm for building LLMs that achieve both low false refusal rates and high safety, offering a flexible solution that decouples behavioral control from core knowledge with minimal computational overhead.
...
<details>
  <summary>Details</summary>
Motivation: LLMs exhibit problematic sensitivity to harmless edits - RAG outputs flip with passage reordering, fine-tuning erodes learned invariances, and compiler optimizations perturb logits. These violations break continuous integration and force safety vs speed tradeoffs.

Method: WILSON uses inverse-free curvature mapping over positions/layers (computed with JVPs and Hutchinson probes) combined with activation-level commutators to flag reorder risk. Signals are cheap, model-agnostic for Transformers, and exported as thresholds/CSV artifacts.

Result: The method enables concrete actions: guarding RAG against order effects, catching fine-tuning regressions, stabilizing debate pathways and long contexts, and gating compiler fusions/reorders in deployment.

Conclusion: WILSON helps anticipate failures and approve safe optimizations, allowing reliability and throughput to improve together without architectural or training changes.
...
<details>
  <summary>Details</summary>
Motivation: Rare genetic disease diagnosis faces challenges including insufficient patient data, inaccessible full genome sequencing, and the large number of possible causative genes, leading to prolonged diagnostic journeys and inappropriate treatments, especially in resource-limited settings.

Method: RareNet uses a subgraph-based Graph Neural Network that requires only patient phenotypes to identify the most likely causal gene and retrieve focused patient subgraphs for targeted clinical investigation. It can function standalone or as a pre/post-processing filter for other candidate gene prioritization methods.

Result: Comprehensive evaluation on two biomedical datasets demonstrates competitive and robust causal gene prediction and significant performance gains when integrated with other frameworks.

Conclusion: By requiring only phenotypic data readily available in any clinical setting, RareNet democratizes access to sophisticated genetic analysis, offering particular value for underserved populations lacking advanced genomic infrastructure.
...
<details>
  <summary>Details</summary>
Motivation: Real-world time series exhibit complex non-stationary characteristics that cause distribution shifts, negatively impacting model performance. Existing instance normalization methods don't account for shifts within individual instances.
...
<details>
  <summary>Details</summary>
Motivation: Existing few-shot classifiers based on language-empowered foundation models are vulnerable to poisoning attacks on support datasets, and current defenses lack formal guarantees against adaptive attacks.

Method: LeFCert integrates textual and feature embeddings with adaptive blending, uses a twofold trimmed mean prototype, and provides provable bounds for classification scores. Two variants are proposed: LeFCert-L with randomized smoothing for Lipschitz continuity under dual budget constraints, and LeFCert-C for collective certification with shared poisoning budgets.

Result: Experiments show LeFCert achieves state-of-the-art performance with significant improvements in both clean and certified accuracy compared to existing baselines, while maintaining computational efficiency.

Conclusion: LeFCert provides the first provably robust few-shot classification framework for language-empowered foundation models, offering certified protection against poisoning attacks with practical computational efficiency.
...
<details>
  <summary>Details</summary>
Motivation: Complex high-dimensional data is commonly visualized using 2D scatter plots, but determining their accuracy is challenging. Quality metrics like normalized stress and KL divergence are widely used but are sensitive to uniform scaling, which doesn't actually change the meaningful properties of the visualization.

Method: The authors investigate the scaling effect on stress and KL divergence both analytically and empirically. They introduce a simple technique to make both metrics scale-invariant and test it on a small benchmark.

Result: The study shows how much the metric values change due to scaling and how this affects dimension reduction technique evaluations. The proposed scale-invariant technique accurately captures expected behavior on benchmarks.

Conclusion: Current quality metrics for data visualization projections are unnecessarily sensitive to uniform scaling. The proposed scale-invariant modification provides a more meaningful evaluation of projection quality that focuses on the actual structure rather than arbitrary scaling factors.
...
<details>
  <summary>Details</summary>
Motivation: While linear models show competitive forecasting performance, methods to enhance them remain underexplored. The authors hypothesize that different time series instances may follow heterogeneous linear mappings, motivating the need for adaptive linear models.

Method: Proposes CATS-Linear with Classification Auxiliary Channel-Independence (CACI) that dynamically routes instances to dedicated predictors via classification. Also redesigns trend-seasonal decomposition with a decoupling-linear mapping-recoupling framework for trends and complex-domain linear projections for seasonal components.
...
Conclusion: CATS-Linear successfully enhances linear forecasting models through classification-based instance routing and improved trend-seasonal decomposition, achieving competitive performance without extensive hyperparameter tuning.
...
<details>
  <summary>Details</summary>
Motivation: Large black-box models lack interpretability, making them untrustworthy for high-stakes applications. There's a need for methods that can explain model predictions in terms of original features.

Method: Use SHAP analysis to quantify feature contributions to predictions, then cluster SHAP values to group samples with similar prediction pathways. Includes simulated experiments and Alzheimer's disease case study using ADNI data.

Result: Clustering SHAP values reveals distinct pathways through which different samples arrive at the same predictions, providing deeper insights into model behavior and data patterns.

Conclusion: SHAP value clustering offers a powerful approach to understand black-box models by identifying similar prediction mechanisms across samples, enhancing model interpretability and trustworthiness.
...
<details>
  <summary>Details</summary>
Motivation: Traditional genomic selection methods have limited accuracy for complex traits and large datasets, creating a need for more advanced approaches to improve crop breeding efficiency.

Method: DPCformer integrates convolutional neural networks with self-attention mechanisms, uses 8-dimensional one-hot encoding for SNP data ordered by chromosome, and employs PMF algorithm for feature selection.

Result: DPCformer significantly outperformed existing methods: maize traits improved by up to 2.92%, cotton fiber traits by 8.37%, tomato small-sample scenarios by 57.35% in Pearson Correlation, and chickpea yield by 16.62%.
...
<details>
  <summary>Details</summary>
Motivation: Spatial causal inference faces dual challenges: unmeasured spatial confounders and treatment interference. Existing methods typically handle only one issue while assuming the other away, creating limitations for real-world applications.

Method: A two-stage approach using a conditional variational autoencoder (CVAE) with spatial prior to reconstruct substitute confounders from local treatment vectors, followed by flexible outcome modeling for causal effect estimation.

Result: The method enables nonparametric identification of direct and spillover effects under weak assumptions, without requiring multiple treatment types or known latent field models. Empirical tests on extended SpaCE benchmark show consistent improvement in effect estimation across environmental health and social science datasets.

Conclusion: By leveraging interference as a multi-cause signal, the Spatial Deconfounder bridges spatial and deconfounding literatures, advancing robust causal inference in structured spatial data.
...
<details>
  <summary>Details</summary>
Motivation: Existing feature caching methods assume feature continuity across timesteps, but this doesn't always hold. Analysis reveals different frequency bands exhibit different dynamics: low-frequency components have high similarity but poor continuity, while high-frequency components show good continuity but poor similarity.

Method: Proposes Frequency-aware Caching (FreqCa) that reuses low-frequency features directly based on similarity and predicts high-frequency features using a second-order Hermite interpolator based on continuity. Also introduces caching Cumulative Residual Feature (CRF) instead of all layer features.
...
Conclusion: FreqCa successfully addresses diffusion transformer inference costs by leveraging frequency domain insights, achieving significant memory reduction (99%) while maintaining performance across multiple models and tasks.
...
<details>
  <summary>Details</summary>
Motivation: Existing bandit analyses focus on time-domain and cumulative regret perspectives, which struggle to characterize the dynamic learning process. The authors aim to provide a more intuitive understanding of bandit algorithms through frequency-domain analysis.

Method: The paper proposes a frequency-domain analysis framework that reformulates the bandit process as a signal processing problem. Reward estimates are viewed as spectral components, uncertainty corresponds to frequency, and bandit algorithms are interpreted as adaptive filters. They construct a formal Frequency-Domain Bandit Model and prove equivalence between UCB's confidence bound and time-varying gain in frequency domain.

Result: The main theorem proves that the confidence bound term in UCB is equivalent to a time-varying gain applied to uncertain spectral components, inversely proportional to the square root of visit count. The authors derive finite-time dynamic bounds for exploration rate decay.

Conclusion: The frequency-domain framework provides novel physical interpretation for classical bandit algorithms and lays theoretical foundation for designing next-generation algorithms with adaptive parameter adjustment, offering deeper insights into the exploration-exploitation trade-off.
...
<details>
  <summary>Details</summary>
Motivation: Standard GRPO wastes substantial compute on negative groups where no sampled response is correct, yielding zero advantage and no gradient. The paper aims to leverage these negative groups without extra supervision.

Method: Starting from maximum-likelihood objective in reward modeling, the authors show MLE gradient is equivalent to policy gradient for a modified value function that adds confidence-weighted penalties on incorrect responses. LENS modifies GRPO to assign non-zero, confidence-dependent rewards to incorrect generations.
...
Conclusion: LENS provides a principled and practical way to "rescue" negative groups, improving efficiency and performance in reinforcement learning with verifiable rewards.
...
<details>
  <summary>Details</summary>
Motivation: Standard supervised learning requires costly retraining for new tasks, and existing ICL-based equalizers only handle static channels. There's a gap in principled analysis and theoretical studies for non-stationary settings where functions change within the context window.

Method: The authors employ a principled framework for designing efficient attention mechanisms with improved adaptivity, leveraging algorithms from adaptive signal processing. They derive new attention variants from LMS, LRMS for robustness, and multi-step gradient updates for better long-term tracking.
...
Conclusion: The findings provide critical insights for developing next-generation wireless foundation models with stronger adaptability and robustness, demonstrating that properly designed attention mechanisms can effectively handle time-varying channel equalization problems.
...
<details>
  <summary>Details</summary>
Motivation: To learn meaningful representations without labeled data by treating individual instances as distinct classes, enabling the model to capture underlying object invariance across transformed views.

Method: Generate two different views of each instance through stochastic transformations, then train the model to recognize that these transformed views belong to the same original instance class.

Result: The model learns representations that are invariant to the applied transformations, capturing the common underlying object characteristics across different views of the same instance.

Conclusion: Instance discrimination provides an effective self-supervised learning approach where treating instances as classes and using view transformations enables learning of transformation-invariant representations without requiring manual labels.
...
<details>
  <summary>Details</summary>
Motivation: Counterfactual fairness for prediction sets is underexplored, while conformal prediction provides valid sets but lacks fairness guarantees.

Method: Symmetrization of conformity scores across protected-attribute interventions to create counterfactually fair prediction sets.

Result: CF-CP achieves counterfactual fairness and target coverage with minimal size increase on synthetic and real datasets.

Conclusion: CF-CP provides a simple, training-free approach for counterfactually fair uncertainty quantification.
...
<details>
  <summary>Details</summary>
Motivation: Existing techniques for controlling LLM behavior through internal state modifications (vector additions or weight updates) are guided by empirical heuristics without strong theoretical foundation, creating a need for principled understanding of how these interventions work.

Method: The authors generalize the theory of implicit weight updates to deep, multi-block transformers, showing how prompt information is represented internally through weight vectors and matrices, then derive methods for condensing this information into token-independent thought vectors and thought matrices.

Result: The paper demonstrates that textual input can be transmuted into reusable weight updates, providing theoretical explanations for existing vector- and matrix-based model editing techniques.

Conclusion: The work establishes a computational foundation for model steering interventions, offering principled methods for creating reusable control mechanisms from text inputs that explain and improve upon existing empirical techniques.
...
<details>
  <summary>Details</summary>
Motivation: Existing in-context learning approaches have limited success in molecular design, and labeled data for molecular properties remains scarce despite large databases like ChEMBL containing millions of biological assays.

Method: DemoDiff uses demonstration-conditioned diffusion models that define task contexts using small sets of molecule-score examples. It employs a novel molecular tokenizer with Node Pair Encoding for motif-level representation, requiring 5.5× fewer nodes. The model is pretrained on a curated dataset containing millions of context tasks from multiple sources.

Result: Across 33 design tasks in six categories, DemoDiff matches or surpasses language models 100-1000× larger and achieves an average rank of 3.63 compared to 5.25-10.20 for domain-specific approaches.

Conclusion: DemoDiff positions itself as a molecular foundation model for in-context molecular design, demonstrating superior performance while being more computationally efficient than existing approaches.
...
### [37] [RFOD: Random Forest-based Outlier Detection for Tabular Data](https://arxiv.org/abs/2510.08747)
...
<details>
  <summary>Details</summary>
Motivation: Existing outlier detection methods struggle with mixed-type tabular data, often losing semantic information through encoding schemes and lacking interpretability for identifying specific anomalous values.

Method: RFOD trains dedicated random forests for each feature conditioned on others, uses Adjusted Gower's Distance for cell-level scoring that adapts to skewed numerical data and categorical confidence, and employs Uncertainty-Weighted Averaging to aggregate scores into robust row-level anomaly scores.
...
Conclusion: RFOD provides an effective solution for outlier detection in mixed-type tabular data, addressing key limitations of existing methods through its feature-wise conditional reconstruction approach and interpretable scoring mechanisms.
...
<details>
  <summary>Details</summary>
Motivation: Deep learning models lack provable guarantees on risk and reliability, which is critical for high-stakes applications. Existing conformal risk control methods only address expected loss but fail to handle tail risks that are important in many real-world scenarios.

Method: The authors develop a method for controlling OCE risks (including CVaR) and introduce 'conformal risk training' - an end-to-end approach that differentiates through conformal OCE risk control during model training or fine-tuning, providing feedback to improve average-case performance.

Result: The method achieves provable risk guarantees while demonstrating significantly improved average-case performance over post-hoc approaches in applications including controlling classifiers' false negative rate and financial risk in battery storage operation.

Conclusion: The proposed approach successfully extends risk control beyond expected loss to handle tail risks through OCE risk measures, and the end-to-end training method overcomes limitations of post-hoc approaches by maintaining provable guarantees while improving overall model performance.
...
<details>
  <summary>Details</summary>
Motivation: Existing FL memorization detection techniques focus on individual samples and underestimate cross-sample memorization risks, while centralized learning methods cannot be directly applied to FL due to data decentralization.

Method: The authors develop a framework that measures fine-grained cross-sample memorization across all clients in FL, enabling quantification of both intra-client (within same client) and inter-client (across different clients) memorization.

Result: FL models do memorize client data, with intra-client data being memorized more than inter-client data. Memorization is influenced by training and inferencing factors including decoding strategies, prefix length, and FL algorithms.

Conclusion: The proposed framework successfully bridges the gap between centralized and federated learning memorization analysis, revealing that FL models exhibit significant memorization patterns that are influenced by various training and inference parameters.
...
<details>
  <summary>Details</summary>
Motivation: Optimizing neural networks for quantized objectives is challenging due to the piece-wise constant nature of quantizers, which yields zero gradients everywhere except at undefined quantization thresholds. Existing methods like Straight Through Estimators (STE) lack convergence guarantees.

Method: LOTION approximates the quantized loss surface with a continuous loss surface using stochastic-noise smoothing. It replaces the raw quantized loss with its expectation under unbiased randomized-rounding noise, allowing standard optimizers to converge to local minima while preserving global minima when using stochastic rounding noise.

Result: The method empirically outperforms standard Quantization-Aware Training (QAT) on synthetic testbeds and on 150M- and 300M-parameter language models, demonstrating improved performance in quantized optimization.

Conclusion: LOTION provides a principled smoothing framework that enables guaranteed convergence in quantized neural network optimization while preserving the global minima of the original quantized loss, offering superior performance compared to existing methods.
...
### [41] [Reinforcement Learning-Based Optimization of CT Acquisition and Reconstruction Parameters Through Virtual Imaging Trials](https://arxiv.org/abs/2510.08763)
...
Method: Combined virtual imaging tools with reinforcement learning using PPO agent to optimize CT protocols. Used human models with liver lesions imaged via CT simulator and reconstructed with novel toolkit. Parameter space included tube voltage, tube current, reconstruction kernel, slice thickness, and pixel size.

Result: The reinforcement learning approach achieved the global maximum detectability index (d') across test cases while requiring 79.7% fewer steps than exhaustive search performed on a supercomputer.

Conclusion: The framework is flexible for various image quality objectives and demonstrates the potential of integrating virtual imaging tools with reinforcement learning for efficient CT protocol management.
...
<details>
  <summary>Details</summary>
Motivation: RL policies often fail to generalize to new robots, tasks, or environments with different physical parameters, limiting real-world applicability.

Method: Based on Buckingham's Pi Theorem, the method scales policy inputs (observations) and outputs (actions) through a dimensionless space for zero-shot transfer without retraining.

Result: Scaled transfer shows no performance loss on dynamically similar contexts and consistently outperforms naive transfer on non-similar contexts, significantly expanding effective context volume.
...
<details>
  <summary>Details</summary>
Motivation: Current LLM embeddings operate on raw text only, missing rich structural information that provides crucial context in real-world datasets with hyperlinks, citations, etc.

Method: Two in-process methods: sequential concatenation and parallel caching, plus techniques for noisy data (Context Distillation and Semantic Balancing). Evaluated through zero-shot experiments on retrieval, clustering, classification, and recommendation.

Result: Structure-aware approaches consistently outperform text-only and post-hoc baselines. Sequential concatenation works best with noisy, moderate-length contexts, while parallel caching scales better to long, high-signal contexts but is more susceptible to distractors.

Conclusion: Provides first comprehensive analysis of in-process structure-aware encoding, offering blueprint for building more powerful and contextually aware embedding models.
...
Method: Provide LLM-generated action recommendations through augmented observation spaces, allowing RL agents to learn when to follow or ignore guidance using soft constraints without modifying existing RL algorithms.

Result: 71% relative improvement in final success rates in the most challenging environment, with agents reaching performance thresholds up to 9 times faster, and benefits scaling with task difficulty.

Conclusion: The approach effectively leverages LLM planning capabilities to accelerate RL training in challenging sparse-reward environments while maintaining flexibility through soft constraints.
...
<details>
  <summary>Details</summary>
Motivation: To address challenges in neural function approximation: need for training from scratch for each function, sensitivity to architecture/hyperparameters, and poor generalization beyond training domains.

Method: Basis neural networks pretrained on polynomial families in a reference domain, then used to initialize networks for target functions, with domain mapping to transform inputs to reference domain.
...
Conclusion: Initialization-based strategies using basis function pretraining offer promise for scalable and modular neural function approximation.
...
<details>
  <summary>Details</summary>
Motivation: To address scenarios where agents need to optimize private objectives while maintaining plausible deniability about their true intentions from observers who can only monitor public actions and outcomes.

Method: Formalizes detectability as stepwise KL divergence constraints, models pulling of public suboptimal arms as a Bernoulli process, formulates a maximin problem for optimal error exponent, and proposes a top-two inspired algorithm that adapts exploration based on public suboptimality gaps.

Result: Shows that pulling public suboptimal arms can happen at most at Θ(√T) rate under KL constraints, characterizes optimal error exponent for best private arm identification, and provides numerical validation of the theoretical rates.

Conclusion: The proposed framework successfully balances the trade-off between efficient private arm identification and maintaining undetectability from observers monitoring public behavior, with theoretical guarantees on detection rates.
...
<details>
  <summary>Details</summary>
Motivation: To develop a more efficient and accurate framework for learning solution operators of partial differential equations that can handle varying inputs (parameters, initial/boundary conditions) while maintaining physical consistency through physics-informed constraints.

Method: Combines DeepONet architecture with Chunkwise Rational Kolmogorov-Arnold Network (CKAN) sub-networks and integrates Physics-Informed Neural Networks (PINNs) principles to enforce physical consistency through PDE residual loss.

Result: On Burgers' equation with ν=0.01, PO-CKAN reduces mean relative L² error by approximately 48% compared to PI-DeepONet, and achieves competitive accuracy on Eikonal and diffusion-reaction benchmarks, closely matching high-fidelity solutions.

Conclusion: PO-CKAN demonstrates superior performance in operator learning for parametric time-dependent PDEs, offering enhanced accuracy and physical consistency while enabling rapid predictions for varying inputs after training.
...
<details>
  <summary>Details</summary>
Motivation: AI-powered attacks on LWE show promise but are limited by scarce training data, which is difficult and resource-intensive to create due to domain expertise requirements and computational costs.

Method: The authors developed TAPAS (Toolkit for Analysis of Post-quantum cryptography using AI Systems), a collection of datasets covering various LWE settings that can be used directly by AI practitioners.

Result: The work provides documented dataset creation processes, establishes performance baselines for LWE attacks, and creates accessible resources for AI research in post-quantum cryptography.

Conclusion: TAPAS datasets fill a critical gap in AI research on LWE attacks and provide a foundation for future work in developing more effective AI-based cryptographic attacks.
...
<details>
  <summary>Details</summary>
Motivation: Existing emotion recognition methods use static fusion strategies and assume consistent modality reliability, which doesn't reflect real-world learning environments where modalities may be missing or noisy.

Method: The framework includes three key components: Cross-Modality Attention Alignment for dynamic cross-modal context sharing, Modality Importance Estimator for confidence-based weighting of modalities at each time step, and Temporal Feedback Loop to leverage previous predictions for temporal consistency.

Result: Edu-EmotionNet achieves state-of-the-art performance on educational subsets of IEMOCAP and MOSEI datasets, re-annotated for educational emotions (confusion, curiosity, boredom, frustration), and demonstrates strong robustness to missing or noisy modalities.

Conclusion: The model effectively captures emotional transitions and adaptively prioritizes reliable signals, making it suitable for deployment in real-time learning systems.
...
<details>
  <summary>Details</summary>
Motivation: To explore whether compact, resource-efficient language models can perform structural inference on graph data, as this capability remains largely unexplored despite LLMs showing emerging reasoning abilities.

Method: Created TinyGraphEstimator dataset with connected graphs from multiple random models, then evaluated small open models on predicting graph parameters (density, clustering, chromatic number) and applied LoRA fine-tuning.
...
Conclusion: Compact transformer models possess meaningful graph reasoning abilities and can be effectively adapted for structural inference tasks through efficient parameter tuning methods like LoRA.
...
<details>
  <summary>Details</summary>
Motivation: Real-world data distributions are often imbalanced (long-tailed), causing deep classification models to be biased toward frequent classes and perform poorly on tail classes. This paper aims to address this majority-biased tendency while maintaining valuable dataset information.

Method: Two-stage approach: 1) Information theory-based representation learning that minimizes intra-class distance for effective feature separation; 2) Novel sampling strategy that selects mathematically informative instances to rectify biased decision boundaries without compromising overall performance.

Result: Achieves state-of-the-art performance across various long-tailed benchmark datasets through extensive experimental validation.

Conclusion: The proposed two-stage learning approach effectively mitigates majority bias in long-tailed classification while preserving dataset information, demonstrating superior performance on benchmark datasets.
...
<details>
  <summary>Details</summary>
Motivation: Real-time multi-view 3D reconstruction is critical for edge-native applications like fire rescue, but dynamic edge resource availability introduces disruptions that challenge reconstruction reliability.

Method: Two cooperative Q-learning agents for camera selection and server selection operating entirely online, learning policies through interactions with edge environment. Implemented on distributed testbed with lab-hosted devices and FABRIC infrastructure.
...
Conclusion: The RL-based framework enables high-quality 3D reconstruction within reasonable time despite resource-constrained and disruption-prone edge environments.
...
<details>
  <summary>Details</summary>
Motivation: Current ML fairness approaches focus mainly on medical image diagnosis tasks like classification and segmentation, but overlook prognosis scenarios involving time-to-event predictions, creating a significant gap in ensuring equitable medical AI systems.

Method: The FairTTE framework integrates cutting-edge TTE prediction and fairness algorithms, uses causal analysis techniques to uncover bias sources, and conducts large-scale evaluation across diverse imaging modalities to systematically analyze fairness in medical image prognosis.

Result: The evaluation reveals pervasive bias across different imaging modalities, limited effectiveness of current fairness methods, strong association between bias sources and model disparities, and increasing difficulty maintaining fairness under distribution shifts.
...
Method: Analyzed representation alignment of CL and NSCL models trained under shared randomness (same initialization, batches, and augmentations), proving similarity matrices stay close under realistic conditions with high-probability guarantees on alignment metrics like CKA and RSA.

Result: Showed that CL-NSCL alignment strengthens with scale and temperature, and that NSCL tracks CL more closely than other supervised objectives, while parameter-space coupling is inherently unstable with weights potentially diverging exponentially.

Conclusion: NSCL serves as a principled bridge between self-supervised and supervised learning, with strong representation-level alignment despite parameter divergence, providing theoretical guarantees for alignment metrics and clarifying how alignment improves with various factors.
...
### [55] [Time-Aware Feature Selection: Adaptive Temporal Masking for Stable Sparse Autoencoder Training](https://arxiv.org/abs/2510.08855)
*T. Ed Li,Junyu Ren*

Main category: cs.LG

TL;DR: ATM (Adaptive Temporal Masking) is a novel training method for sparse autoencoders that reduces feature absorption by dynamically adjusting feature selection using activation magnitudes, frequencies, and reconstruction contributions, achieving better interpretability while maintaining reconstruction quality.


<details>
  <summary>Details</summary>
Motivation: Current sparse autoencoder training methods suffer from feature absorption, where features merge to minimize L1 penalty, making it difficult to consistently identify and analyze model behaviors for reliability and safety.

Method: ATM tracks activation magnitudes, frequencies, and reconstruction contributions to compute evolving importance scores, then applies probabilistic masking based on statistical thresholding of these scores for more natural feature selection.

Result: Extensive experiments on Gemma-2-2b model show ATM achieves substantially lower absorption scores compared to TopK and JumpReLU SAEs while maintaining excellent reconstruction quality.
...
### [56] [Sparse components distinguish visual pathways & their alignment to neural networks](https://arxiv.org/abs/2510.08858)
*Ammar I Marvi,Nancy G Kanwisher,Meenakshi Khosla*

Main category: cs.LG

TL;DR: A novel sparse decomposition approach reveals distinct functional components across human visual streams (ventral, dorsal, lateral), and introduces Sparse Component Alignment (SCA) to better measure representational alignment between brains and deep neural networks.


<details>
  <summary>Details</summary>
Motivation: To resolve the inconsistency between traditional neuroscience findings of distinct functional processes in different visual streams and the surprising ability of single-task DNNs to model the entire visual system well, suggesting common computational principles.

Method: Applied sparse decomposition to identify dominant components in each visual stream, then introduced Sparse Component Alignment (SCA) - a new method for measuring representational alignment between brains and machines that captures latent neural tuning.

Result: Found clear differences in component response profiles: ventral stream selective for faces, places, bodies, text, food; lateral stream for social interactions, implied motion, hand actions; dorsal stream had less interpretable components. SCA revealed standard DNNs are more aligned with ventral than dorsal or lateral representations.

Conclusion: SCA provides higher-resolution distinction of representational alignment than conventional population-level geometry, offering a measure sensitive to a system's underlying axes of neural tuning, revealing DNNs' stronger alignment with ventral stream representations.
...
### [57] [Multi-fidelity Batch Active Learning for Gaussian Process Classifiers](https://arxiv.org/abs/2510.08865)
*Murray Cutforth,Yiming Yang,Tiffany Fan,Serge Guillas,Eric Darve*

Main category: cs.LG

TL;DR: BPMI is a batch active learning algorithm for multi-fidelity GP classifiers that uses Bernoulli Parameter Mutual Information to efficiently allocate simulation budgets by approximating mutual information through Taylor expansion.


<details>
  <summary>Details</summary>
Motivation: Many science and engineering problems rely on expensive computational simulations, and a multi-fidelity approach can accelerate parameter space exploration. There's a need for efficient budget allocation in binary simulation output cases using Gaussian Process models.

Method: Introduces Bernoulli Parameter Mutual Information (BPMI), which circumvents the intractability of calculating mutual information in probability space by employing a first-order Taylor expansion of the link function. This is a batch active learning algorithm for multi-fidelity GP classifiers.

Result: BPMI was evaluated against several baselines on two synthetic test cases and a complex real-world application involving laser-ignited rocket combustor simulation. In all experiments, BPMI demonstrated superior performance.

Conclusion: BPMI achieves higher predictive accuracy for a fixed computational budget compared to baseline methods, making it an effective approach for multi-fidelity simulation budget allocation in binary output scenarios.
...
### [58] [An Improved Model-Free Decision-Estimation Coefficient with Applications in Adversarial MDPs](https://arxiv.org/abs/2510.08882)
*Haolin Liu,Chen-Yu Wei,Julian Zimmert*

Main category: cs.LG

TL;DR: The paper introduces Dig-DEC, a model-free decision-estimation coefficient that uses information gain for exploration instead of optimism. It achieves tighter regret bounds than previous methods and works in adversarial environments, resolving open problems in hybrid MDPs with bandit feedback.


<details>
  <summary>Details</summary>
Motivation: To close the gap between regret upper and lower bounds in DMSO that scales with model class size, and to extend optimism-based exploration methods to handle adversarial settings where they were previously limited to stochastic environments.

Method: Proposed Dig-DEC, which removes optimism and uses pure information gain for exploration. Applied to hybrid MDPs with stochastic transitions and adversarial rewards, and improved online function-estimation procedures for both average estimation error minimization and squared error minimization in Bellman-complete MDPs.

Result: Dig-DEC is always no larger than optimistic DEC and can be much smaller. Achieved first model-free regret bounds for hybrid MDPs with bandit feedback under general transition structures. Improved regret bounds from T^{3/4} to T^{2/3} (on-policy) and from T^{5/6} to T^{7/9} (off-policy) for average estimation error, and from T^{2/3} to √T for squared error minimization in Bellman-complete MDPs.

Conclusion: Dig-DEC successfully addresses limitations of optimistic DEC by enabling adversarial environment handling without explicit reward estimators, achieves tighter regret bounds, and matches performance of optimism-based approaches in Bellman-complete MDPs for the first time.
...
### [59] [Pinpointing crucial steps: Attribution-based Credit Assignment for Verifiable Reinforcement Learning](https://arxiv.org/abs/2510.08899)
*Junxi Yin,Haisen Luo,Zhenyu Li,Yihua Liu,Dan Liu,Zequn Li,Xiaohang Xu*

Main category: cs.LG

TL;DR: ACPO is a phased RL framework that addresses exploration-exploitation imbalance in LLMs through difficulty-aware curriculum, trajectory segmentation, and factorized rewards for better credit assignment.


<details>
  <summary>Details</summary>
Motivation: Current RLVR methods struggle with balancing exploration and exploitation, leading to inaccurate credit assignment for intermediate reasoning steps and premature entropy collapse that limits model performance.
...
Result: Extensive experiments on AIME, MATH, and AMC benchmarks show ACPO significantly outperforms state-of-the-art approaches.

Conclusion: ACPO effectively addresses exploration-exploitation imbalance in RLVR through its phased framework, improving both exploration via entropy regulation and exploitation via precise credit assignment.
...
### [60] [Velocity and Density-Aware RRI Analysis and Optimization for AoI Minimization in IoV SPS](https://arxiv.org/abs/2510.08911)
*Maoxin Ji,Tong Wang,Qiong Wu,Pingyi Fan,Nan Cheng,Wen Chen*

Main category: cs.LG

TL;DR: Proposes LLM+DDPG optimization to reduce Age of Information in IoV by addressing packet collisions and speed-related channel uncertainties in SPS scheduling.


<details>
  <summary>Details</summary>
Motivation: Address AoI deterioration caused by packet collisions and vehicle speed-related channel uncertainties in Semi-Persistent Scheduling for Internet of Vehicles.

Method: Establishes AoI calculation model considering vehicle speed, density, and RRI; designs dual-path optimization with DDPG (guided by state space and reward function) and LLM (using contextual learning for parameter configurations).

Result: LLM significantly reduces AoI after few exemplars without training; DDPG achieves more stable performance after training.

Conclusion: Combined LLM and DDPG approach effectively optimizes AoI in IoV SPS systems, with LLM providing quick optimization and DDPG offering stable performance.
...
### [61] [Simple and Robust Forecasting of Spatiotemporally Correlated Small Earth Data with A Tabular Foundation Model](https://arxiv.org/abs/2510.08920)
*Yuting Yang,Gang Mei,Zhengjing Ma,Nengxiong Xu,Jianbing Peng*

Main category: cs.LG

TL;DR: A novel approach for spatiotemporal forecasting of small Earth data that characterizes spatiotemporal patterns and uses tabular foundation models, achieving superior accuracy and robustness compared to existing methods.


<details>
  <summary>Details</summary>
Motivation: Small Earth data with limited short-term monitoring variability and spatiotemporal correlations are crucial for understanding geoscientific processes, but conventional deep learning models require task-specific training while foundation models exhibit forecasting bias toward global means.

Method: Characterize and quantify spatiotemporal patterns of small Earth data, then utilize tabular foundation models for accurate forecasting across different scenarios without task-specific training.

Result: The proposed approach achieves superior accuracy compared to graph deep learning model (T-GCN) and tabular foundation model (TabPFN) in most instances across three typical scenarios, demonstrating stronger robustness.

Conclusion: The simple and robust approach effectively addresses the limitations of both conventional deep learning models and foundation models for spatiotemporal forecasting of small Earth data, providing accurate forecasting across different scenarios.
...
### [62] [AB-PINNs: Adaptive-Basis Physics-Informed Neural Networks for Residual-Driven Domain Decomposition](https://arxiv.org/abs/2510.08924)
*Jonah Botvinick-Greenhouse,Wael H. Ali,Mouhacine Benosman,Saviz Mowlavi*

Main category: cs.LG

TL;DR: AB-PINNs introduce adaptive domain decomposition for physics-informed neural networks, where subdomains dynamically adjust to solution features and new subdomains are added in high-residual regions during training to handle multiscale problems.


<details>
  <summary>Details</summary>
Motivation: To address the challenges of training PINNs for complex multiscale PDEs by providing flexible domain decomposition that adapts to solution features and prevents convergence to local minima.

Method: Dynamic domain decomposition where existing subdomains adapt to solution features and new subdomains are introduced in regions of high residual loss during training, inspired by classical mesh refinement techniques.

Result: Comprehensive numerical results demonstrate AB-PINNs effectively solve various complex multiscale partial differential equations, showing improved performance over static approaches.

Conclusion: AB-PINNs provide a flexible and effective approach for solving multiscale PDEs by combining adaptive domain decomposition with dynamic subdomain creation, reducing hyperparameter tuning needs and preventing local minima convergence.
...
### [63] [MATT-CTR: Unleashing a Model-Agnostic Test-Time Paradigm for CTR Prediction with Confidence-Guided Inference Paths](https://arxiv.org/abs/2510.08932)
*Moyu Zhang,Yun Chen,Yujun Jin,Jinxin Hu,Yu Zhang,Xiaoyi Zeng*

Main category: cs.LG

TL;DR: MATT is a model-agnostic test-time paradigm that improves CTR prediction by generating multiple inference paths based on feature combination confidence scores, mitigating the impact of low-confidence features.


<details>
  <summary>Details</summary>
Motivation: Previous CTR research focused mainly on training phase optimization, neglecting inference phase opportunities. Infrequent feature combinations degrade prediction performance, leading to unreliable outputs.

Method: Uses hierarchical probabilistic hashing to estimate feature combination frequencies as confidence scores, then generates multiple inference paths through iterative sampling based on these scores, and aggregates predictions from all paths.
...
Conclusion: MATT successfully unlocks the predictive potential of trained CTR models by addressing inference-phase optimization through confidence-based path generation and aggregation.
...
### [64] [Bi-level Meta-Policy Control for Dynamic Uncertainty Calibration in Evidential Deep Learning](https://arxiv.org/abs/2510.08938)
*Zhen Yang,Yansong Ma,Lei Chen*

Main category: cs.LG

TL;DR: Proposes Meta-Policy Controller (MPC), a dynamic meta-learning framework that adaptively adjusts KL divergence coefficient and Dirichlet prior strengths for better uncertainty calibration in Evidence Deep Learning, overcoming limitations of static hyperparameters in traditional EDL methods.
...
Conclusion: The proposed MPC framework with learnable Dirichlet prior enables flexible adaptation to class distributions and training dynamics, overcoming limitations of fixed priors in previous methods and achieving superior uncertainty modeling.
...
### [65] [Variability Aware Recursive Neural Network (VARNN): A Residual-Memory Model for Capturing Temporal Deviation in Sequence Regression Modeling](https://arxiv.org/abs/2510.08944)
*Haroon Gharwi,Kai Shu*

Main category: cs.LG

TL;DR: VARNN is a novel neural network architecture that uses error memory from recent prediction residuals to improve time-series regression performance in non-stationary environments with regime shifts and heteroscedastic noise.


<details>
  <summary>Details</summary>
Motivation: Real-world time series data exhibit non-stationary behavior, regime shifts, and temporally varying noise that degrade the robustness of standard regression models.

Method: VARNN augments a feed-forward predictor with a learned error-memory state that is updated from residuals over short context steps and conditions the final prediction at the current time step.

Result: Across diverse dataset domains (appliance energy, healthcare, environmental monitoring), VARNN achieves superior performance with lower test MSE and minimal computational overhead compared to static, dynamic, and recurrent baselines.

Conclusion: VARNN offers robust predictions under drift and volatility environments, highlighting its potential as a promising framework for time-series learning.
...
### [66] [When LLM Agents Meet Graph Optimization: An Automated Data Quality Improvement Approach](https://arxiv.org/abs/2510.08952)
...
<details>
  <summary>Details</summary>
Motivation: Graph neural networks (GNNs) on TAGs are highly sensitive to input quality, suffering significant degradation from sparsity, noise, and imbalance. Existing approaches focus on model architectures but neglect systematic optimization of TAG data itself.

Method: LAGA integrates four collaborative agents: detection, planning, action, and evaluation in an automated closed loop. The action agent uses a dual-encoder and tri-objective design to capture complementary information across modalities for holistic graph quality enhancement.

Result: Experiments across nine scenarios show that LAGA improves graph quality and achieves state-of-the-art performance across various tasks and backbones.

Conclusion: Data-centric quality optimization is key to reliable TAGs and robust graph learning, with LAGA validating this approach through superior performance in handling graph quality issues.
...
### [67] [Analytical Survey of Learning with Low-Resource Data: From Analysis to Investigation](https://arxiv.org/abs/2510.08962)
...
TL;DR: This survey analyzes learning with low-resource data using PAC framework and active sampling theory, explores optimization strategies for limited data scenarios, and examines various learning paradigms that can benefit from low-resource settings.


<details>
  <summary>Details</summary>
Motivation: High-resource data learning has shown success but comes with significant annotation and training costs. The fundamental goal is to achieve robust generalization with limited-resource data, addressing the practical challenges of data scarcity in AI applications.

Method: Uses agnostic active sampling theory within the PAC framework to analyze generalization error and label complexity for low-resource data in both supervised and unsupervised settings. Investigates optimization strategies including gradient-informed, meta-iteration, geometry-aware, and LLMs-powered optimization.

Result: The analysis provides theoretical understanding of generalization error and label complexity in low-resource settings. The optimization strategies offer practical approaches for improving learning efficiency with limited data.

Conclusion: The survey summarizes key findings and implications for learning with low-resource data, highlighting the importance of specialized optimization techniques and various learning paradigms that can effectively leverage limited data resources.
...
### [68] [HiBBO: HiPPO-based Space Consistency for High-dimensional Bayesian Optimisation](https://arxiv.org/abs/2510.08965)
*Junyu Xuan,Wenlong Chen,Yingzhen Li*

Main category: cs.LG

TL;DR: HiBBO is a novel Bayesian Optimization framework that uses HiPPO to improve latent space consistency in VAE-based approaches, reducing functional distribution mismatch and outperforming existing methods in high-dimensional optimization tasks.


<details>
  <summary>Details</summary>
Motivation: Standard VAE-based Bayesian Optimization methods suffer from functional distribution mismatch between latent and original spaces due to reconstruction-only objectives, leading to suboptimal performance in high-dimensional optimization.

Method: The proposed HiBBO framework introduces space consistency into VAE latent space construction using HiPPO (a long-term sequence modeling method) to better align functional distributions between latent and original spaces.
...
Conclusion: HiBBO successfully bridges high-dimensional sequence representation learning with efficient Bayesian Optimization, enabling broader applications in neural architecture search, materials science, and other domains.
...
### [69] [Learning Regularizers: Learning Optimizers that can Regularize](https://arxiv.org/abs/2510.08968)
*Suraj Kumar Sahoo,Narayanan C Krishnan*

Main category: cs.LG

TL;DR: Learned Optimizers can internalize regularization effects without explicit application, outperforming unregularized versions and transferring these benefits to new tasks.


<details>
  <summary>Details</summary>
Motivation: To investigate whether regularizers can be learned by Learned Optimizers rather than explicitly applied, challenging conventional optimization approaches.

Method: Train Learned Optimizers with and without access to explicit regularizers (SAM, GAM, GSAM) and compare performance on standard benchmarks including MNIST, FMNIST, CIFAR datasets using MLP, MLP-Relu and CNN architectures.

Result: Regularized LOs consistently achieve higher test accuracy and better generalization than unregularized counterparts, and can transfer learned regularization effects to new optimization tasks.

Conclusion: LOs can inherently learn and internalize regularization properties, questioning the necessity of explicit loss regularization in optimization.
...
### [70] [Diagnosing and Mitigating System Bias in Self-Rewarding RL](https://arxiv.org/abs/2510.08977)
*Chuyi Tan,Peiwen Yuan,Xinglin Wang,Yiwei Li,Shaoxiong Feng,Yueqi Zhang,Jiayi Shi,Ji Zhang,Boyuan Pan,Yao Hu,Kan Li*

Main category: cs.LG

TL;DR: RLER (Reinforcement Learning with Ensembled Rewards) addresses the bias and instability in RLIR by aggregating diverse models and adapting reward interpolation and rollout selection, achieving performance close to RLVR without requiring labeled data.


<details>
  <summary>Details</summary>
Motivation: RLIR enables sustainable scaling without labeled data but suffers from performance and stability issues due to system bias where models overestimate high-confidence rollouts, leading to biased reward estimation and unstable training.

Method: Proposed RLER which aggregates diverse models and adapts reward interpolation and rollout selection to mitigate the identified system bias characterized by three metrics: ρ_noise, ρ_selfbias, and ρ_symbias.

Result: RLER improves by +13.6% over RLIR and is only 3.6% below RLVR, achieving stable scaling on unlabeled samples.

Conclusion: RLER effectively addresses the bias and instability issues in RLIR, making it highly applicable for reinforcement learning in unlabeled settings while achieving performance comparable to methods requiring labeled data.
...
### [71] [FedL2T: Personalized Federated Learning with Two-Teacher Distillation for Seizure Prediction](https://arxiv.org/abs/2510.08984)
*Jionghao Lou,Jian Zhang,Zhongmei Li,Lanlan Chen,Enbo Feng*

Main category: cs.LG

TL;DR: FedL2T is a personalized federated learning framework for seizure prediction that uses two-teacher knowledge distillation to handle inter-patient variability and improve performance under heterogeneous client settings.


<details>
  <summary>Details</summary>
Motivation: Training deep learning models for seizure prediction requires large labeled EEG datasets, but data acquisition is difficult due to annotation costs and privacy constraints. Existing FL methods struggle with robust performance under heterogeneous client settings due to inter-patient variability.

Method: FedL2T employs a two-teacher knowledge distillation strategy where each client learns from both a globally aggregated model and a dynamically assigned peer model. It uses adaptive multi-level distillation aligning prediction outputs and feature representations based on task confidence, plus proximal regularization for training stability.

Result: Extensive experiments on two EEG datasets show FedL2T consistently outperforms state-of-the-art FL methods, especially under low-label conditions. It exhibits rapid and stable convergence, reducing communication rounds and overhead.

Conclusion: FedL2T demonstrates strong potential as a reliable and personalized solution for seizure prediction in privacy-sensitive healthcare scenarios, effectively addressing challenges of data heterogeneity and limited labeled data.
...
### [72] [Constraints-of-Thought: A Framework for Constrained Reasoning in Language-Model-Guided Search](https://arxiv.org/abs/2510.08992)
*Kamel Alrashedy,Vriksha Srihari,Zulfiqar Zaidi,Ridam Srivastava,Pradyumna Tambwekar,Matthew Gombolay*

Main category: cs.LG

TL;DR: Constraints-of-Thought (Const-o-T) is a framework that uses (intent, constraint) pairs to guide Monte Carlo Tree Search, improving LLM planning by focusing on semantically meaningful paths while ensuring constraint satisfaction.


<details>
  <summary>Details</summary>
Motivation: LLMs struggle with multi-step planning alignment to user intent and symbolic constraints in complex domains, with existing methods often producing infeasible actions or hallucinated steps.

Method: Proposes Const-o-T framework using (intent, constraint) pairs as structured prior for MCTS, compressing search space and enforcing validity by pruning infeasible branches and guiding exploration toward semantically valid actions.

Result: Outperforms baselines across Risk game, CAD code generation, and arithmetic reasoning domains, yielding higher accuracy and stronger structural alignment.
...
### [73] [PlatformX: An End-to-End Transferable Platform for Energy-Efficient Neural Architecture Search](https://arxiv.org/abs/2510.08993)
*Xiaolong Tu,Dawei Chen,Kyungtae Han,Onur Altintas,Haoxin Wang*

Main category: cs.LG

TL;DR: PlatformX is an automated HW-NAS framework that addresses limitations of existing methods by integrating energy-driven search space, transferable energy prediction, Pareto-based optimization, and automated energy profiling to efficiently find optimal DNN architectures for edge devices.
...
Method: PlatformX integrates four components: energy-driven search space with energy-critical configurations, transferable kernel-level energy predictor refined with minimal samples, Pareto-based multi-objective search algorithm, and automated on-device energy profiling system.
...
Conclusion: PlatformX provides a fully automated and transferable HW-NAS framework that overcomes practical limitations of existing methods, enabling efficient deployment of optimized DNNs across diverse edge devices.
...
### [74] [SQS: Bayesian DNN Compression through Sparse Quantized Sub-distributions](https://arxiv.org/abs/2510.08999)
*Ziyi Wang,Nan Jiang,Guang Lin,Qifan Song*

Main category: cs.LG

TL;DR: A unified framework called SQS that simultaneously performs pruning and low-bit quantization using Bayesian variational learning, achieving higher compression rates than existing methods while maintaining comparable performance.


<details>
  <summary>Details</summary>
Motivation: Existing neural network compression methods use pruning or quantization individually, resulting in suboptimal compression rates. A unified approach is needed to achieve better compression while preserving performance.

Method: Uses Bayesian variational learning with spike-and-slab prior for sparsity and Gaussian Mixture Models (GMMs) for modeling quantized weights, enabling simultaneous pruning and low-bit quantization.

Result: Achieves higher compression rates than existing methods on ResNet, BERT-base, Llama3, and Qwen2.5 models while maintaining comparable performance drops.

Conclusion: The proposed SQS framework provides an effective unified approach for neural network compression that outperforms individual pruning or quantization methods.
...
### [75] [LLM Unlearning on Noisy Forget Sets: A Study of Incomplete, Rewritten, and Watermarked Data](https://arxiv.org/abs/2510.09007)
*Changsheng Wang,Yihua Zhang,Dennis Wei,Jinghan Jia,Pin-Yu Chen,Sijia Liu*

Main category: cs.LG

TL;DR: This paper studies LLM unlearning with noisy forget data and finds it remains surprisingly robust when core semantic signals are preserved, suggesting unlearning algorithms rely on deep semantic cues rather than shallow patterns.


<details>
  <summary>Details</summary>
Motivation: Existing LLM unlearning methods assume access to clean forget data, but real-world forget data could be low-quality, synthetically rewritten, or watermarked, raising concerns about unlearning reliability.
...
Result: Unlearning remains robust to perturbations when core semantic signals are preserved, suggesting algorithms are guided by deep semantic cues rather than shallow lexical patterns.

Conclusion: LLM unlearning shows surprising robustness to noisy forget data, with key semantic components remaining influential despite surface variations, indicating semantic-driven rather than pattern-driven unlearning mechanisms.
...
### [76] [Value-State Gated Attention for Mitigating Extreme-Token Phenomena in Transformers](https://arxiv.org/abs/2510.09017)
*Rui Bu,Haofeng Zhong,Wenzheng Chen,Yangyan Li*

Main category: cs.LG

TL;DR: VGA (Value-State Gated Attention) is a new architectural mechanism that breaks the problematic mutual reinforcement between attention and value states, mitigating attention sinks and value-state drains in Transformers through learnable value-based gating.


<details>
  <summary>Details</summary>
Motivation: Transformer models suffer from extreme-token phenomena like attention sinks and value-state drains, which degrade performance, quantization fidelity, and interpretability due to inefficient 'no-op' behavior where models focus attention on tokens with near-zero value states.

Method: Proposes Value-State Gated Attention (VGA) that introduces a learnable, data-dependent gate computed directly from value vectors to modulate attention output, breaking the mutual reinforcement cycle between attention scores and value states.

Result: VGA significantly mitigates attention sink formation, stabilizes value-state norms, improves model performance, provides robust quantization fidelity, and enhances interpretability compared to prior methods.

Conclusion: Gating attention output directly using value-state information is more effective than previous embedding-based approaches, creating a direct regulatory pathway that efficiently suppresses problematic token contributions while maintaining model stability.
...
### [77] [Slim Scheduler: A Runtime-Aware RL and Scheduler System for Efficient CNN Inference](https://arxiv.org/abs/2510.09018)
*Ian Harshbarger,Calvin Chidambaram*

Main category: cs.LG

TL;DR: Slim Scheduler is a hybrid framework combining PPO reinforcement learning with greedy schedulers to dynamically coordinate distributed inference for slimmable models, achieving significant latency and energy reductions while maintaining accuracy trade-offs.
...
Method: A hybrid scheduling framework integrating Proximal Policy Optimization (PPO) reinforcement learning policy with algorithmic greedy schedulers. Local greedy schedulers batch compatible requests and manage instance scaling, while the PPO router learns global routing policies for device selection, width ratio, and batch configuration.

Result: Achieved 96.45% reduction in mean latency and 97.31% reduction in energy usage when dropping accuracy to the slimmest model (70.3%). Can also reduce average latency plus energy consumption while increasing accuracy, though with higher standard deviations in latency and energy.

Conclusion: The hierarchical design reduces search space complexity, mitigates overfitting to specific hardware, and balances efficiency and throughput, providing flexible accuracy-latency trade-offs for dynamic inference scheduling.
...
### [78] [MagicDock: Toward Docking-oriented De Novo Ligand Design via Gradient Inversion](https://arxiv.org/abs/2510.09020)
*Zekai Chen,Xunkai Li,Sirui Zhang,Henan Sun,Jia Li,Zhenjun Li,Bing Zhou,Rong-Hua Li,Guoren Wang*

Main category: cs.LG

TL;DR: MagicDock is a novel framework for de novo ligand design that addresses limitations in existing methods through gradient inversion and differentiable surface modeling, achieving significant improvements over state-of-the-art baselines.


<details>
  <summary>Details</summary>
Motivation: Existing de novo ligand design methods suffer from Pseudo De Novo approaches, Limited Docking Modeling, and Inflexible Ligand Type constraints, limiting their effectiveness in generating ligands with strong binding affinity from scratch.

Method: Uses a gradient inversion framework that incorporates general docking knowledge into the backbone model, then instantiates this knowledge as reverse gradient flows to guide ligand generation. Employs differentiable surface modeling with learnable 3D point-cloud representations to capture binding details and ensure docking validity through spatial fingerprints. Includes customized designs for different ligand types within a unified framework.

Result: Extensive experiments across 9 scenarios show MagicDock achieves average improvements of 27.1% and 11.7% over state-of-the-art baselines specialized for protein or molecule ligand design respectively.

Conclusion: MagicDock provides a comprehensive solution for de novo ligand design with rigorous theoretical guarantees, overcoming key limitations of existing methods and demonstrating superior performance across diverse scenarios.
...
### [79] [The Environmental Impacts of Machine Learning Training Keep Rising Evidencing Rebound Effect](https://arxiv.org/abs/2510.09022)
*Clément Morand,Anne-Laure Ligozat,Aurélie Névéol*

Main category: cs.LG

TL;DR: The paper analyzes environmental impacts of ML training over the past decade, finding that despite efficiency optimizations, hardware production impacts and training energy consumption have increased exponentially, revealing rebound effects that prevent sustainability.


<details>
  <summary>Details</summary>
Motivation: To investigate whether current optimization strategies (hardware, algorithmic, carbon) can lead to sustainable ML model training given escalating computational demands and environmental concerns.

Method: Estimate environmental impacts of training notable AI systems over the last decade, including Large Language Models, with focus on graphics card life cycle analysis and examination of reduction strategies like location shifting.

Result: Two critical trends: 1) Graphics card production impacts have steadily increased; 2) Training energy consumption and environmental impacts have increased exponentially despite optimization strategies, showing rebound effects that don't mitigate training impacts.

Conclusion: Increasing efficiency alone cannot ensure ML sustainability. Mitigating AI's environmental impact requires reducing AI activities and questioning the scale/frequency of resource-intensive training, while considering hardware impacts over entire life cycle rather than just use phase.
...
### [80] [The Attacker Moves Second: Stronger Adaptive Attacks Bypass Defenses Against Llm Jailbreaks and Prompt Injections](https://arxiv.org/abs/2510.09023)
...
TL;DR: Current language model defense evaluations are flawed. The paper shows that adaptive attackers can bypass 12 recent defenses with >90% success rates, despite original near-zero reported rates.


<details>
  <summary>Details</summary>
Motivation: To address the flawed evaluation process of language model defenses against jailbreaks and prompt injections, which typically use static attack sets or weak optimization methods not designed for the specific defenses.

Method: Systematically tuning and scaling general optimization techniques including gradient descent, reinforcement learning, random search, and human-guided exploration to create adaptive attacks that explicitly counter defense designs.

Result: Successfully bypassed 12 recent defenses with attack success rates above 90% for most defenses, even though the majority originally reported near-zero attack success rates.

Conclusion: Future defense work must consider stronger adaptive attacks to make reliable and convincing claims of robustness, as current evaluation methods are insufficient.
...
### [81] [Convergence of optimizers implies eigenvalues filtering at equilibrium](https://arxiv.org/abs/2510.09034)
*Jerome Bolte,Quoc-Tung Le,Edouard Pauwels*

Main category: cs.LG

TL;DR: The paper analyzes how different optimizers act as eigenvalue filters that influence which minima are found, with gradient descent avoiding sharp minima and SAM favoring wider basins. The authors propose two new algorithms with enhanced eigenvalue filtering and provide theoretical analysis using stable manifold theory.


<details>
  <summary>Details</summary>
Motivation: Empirical evidence shows various optimizers find nearly global optima, so the paper takes the perspective of assuming convergence and studying the consequences, particularly how optimizers act as eigenvalue filters determined by their hyperparameters.

Method: Theoretical analysis using generalized Hadamard-Perron stable manifold theorem applied to general semialgebraic C² functions, without requiring non-degeneracy conditions or global Lipschitz bounds. Numerical experiments on feed-forward neural networks support the conclusions.

Result: Standard gradient descent inherently avoids sharpest minima, while SAM algorithms actively favor wider basins. The proposed novel algorithms exhibit enhanced eigenvalue filtering that effectively promotes wider minima.

Conclusion: Different optimizers effectively act as eigenvalue filters determined by their hyperparameters, with gradient descent avoiding sharp minima and SAM favoring wider basins. The proposed algorithms with enhanced eigenvalue filtering successfully promote wider minima.
...
### [82] [Robust Driving Control for Autonomous Vehicles: An Intelligent General-sum Constrained Adversarial Reinforcement Learning Approach](https://arxiv.org/abs/2510.09041)
*Junchao Fan,Xiaolin Chang*

Main category: cs.LG

TL;DR: IGCARL is a novel robust autonomous driving approach that uses strategic adversarial attacks and constrained reinforcement learning to improve DRL's resilience against sophisticated threats while maintaining training stability.


<details>
  <summary>Details</summary>
Motivation: Current robust DRL methods for autonomous driving have three key limitations: they only handle short-sighted attacks, fail to cause truly safety-critical events, and suffer from training instability due to lack of robust constraints.

Method: IGCARL consists of two components: (1) a strategic targeted adversary that performs coordinated multi-step attacks using DRL's temporal decision-making and focuses on inducing safety-critical events with general-sum objectives, and (2) a robust driving agent trained under constrained formulation to ensure stable learning against adversarial attacks.

Result: Extensive experiments show IGCARL improves the success rate by at least 27.9% over state-of-the-art methods, demonstrating superior robustness to adversarial attacks.

Conclusion: IGCARL enhances the safety and reliability of DRL-based autonomous driving by effectively addressing key vulnerabilities through strategic adversarial training and constrained optimization.
...
### [83] [Spatio-Temporal Graph Convolutional Networks for EV Charging Demand Forecasting Using Real-World Multi-Modal Data Integration](https://arxiv.org/abs/2510.09048)
*Jose Tupayachi,Mustafa C. Camur,Kevin Heaslip,Xueping Li*

Main category: cs.LG

TL;DR: TW-GCN is a spatio-temporal forecasting framework that combines Graph Convolutional Networks with temporal architectures to predict EV charging demand in Tennessee, achieving best performance with 3-hour forecasts and 1DCNN temporal models.


<details>
  <summary>Details</summary>
Motivation: Transportation's major contribution to greenhouse gas emissions creates urgency for EV transition, but uneven charging infrastructure distribution and utilization pose challenges for grid stability and investment planning.

Method: Combines Graph Convolutional Networks with temporal architectures using real-world traffic flows, weather conditions, and proprietary EV infrastructure data to capture spatial dependencies and temporal dynamics.
...
Conclusion: The TW-GCN framework advances integration of data-driven intelligence into EV infrastructure planning, supporting sustainable mobility transitions and resilient grid management.
...
### [84] [Improving Anomaly Detection in Industrial Time Series: The Role of Segmentation and Heterogeneous Ensemble](https://arxiv.org/abs/2510.09079)
*Emilio Mastriani,Alessandro Costa,Federico Incardona,Kevin Munari,Sebastiano Spinello*

Main category: cs.LG

TL;DR: Segmentation techniques combined with heterogeneous ensembles improve anomaly detection in industrial production, boosting AUC-ROC from 0.8599 to 0.9760 by reducing temporal uncertainty.


<details>
  <summary>Details</summary>
Motivation: To enhance anomaly detection in industrial production by addressing temporal uncertainty through segmentation techniques that identify state changes in time series data.

Method: Integration of segmentation techniques (Change Point Detection algorithms like ChangeFinder) as pre-processing step before applying heterogeneous ensemble algorithms (Random Forest and XGBoost).

Result: Significant improvement in AUC-ROC metric from 0.8599 (PCA and LSTM ensemble) to 0.9760 (Random Forest and XGBoost with segmentation pre-processing).

Conclusion: Segmentation as pre-processing effectively reduces temporal ambiguity and facilitates supervised learning, providing substantial performance gains for anomaly detection in industrial contexts.
...
### [85] [FLToP CTC: Frame-Level Token Pruning via Relative Threshold for Efficient and Memory-Saving Decoding on Diverse Platforms](https://arxiv.org/abs/2510.09085)
...
<details>
  <summary>Details</summary>
Motivation: CTC-based ASR systems face computational and memory bottlenecks in resource-limited environments, with traditional CTC decoders requiring up to 90% of processing time due to exhaustive token-level operations.

Method: The paper introduces Frame Level Token Pruning for Connectionist Temporal Classification (FLToP CTC), which employs frame-level token pruning guided by a relative threshold probability to dynamically eliminate low-probability tokens per frame.
...
Conclusion: FLToP CTC addresses CTC bottlenecks, offering scalability for resource-limited environments and realtime applications, enhancing speech recognition accessibility and efficiency through its simple integration across platforms.
...
### [86] [Neural Codecs as Biosignal Tokenizers](https://arxiv.org/abs/2510.09095)
*Kleanthis Avramidis,Tiantian Feng,Woojae Jeong,Jihwan Lee,Wenhui Cui,Richard M Leahy,Shrikanth Narayanan*

Main category: cs.LG

TL;DR: BioCodec is a representation learning framework inspired by neural codecs that converts high-dimensional, noisy biosignals like EEG into discrete tokens, enabling effective downstream task performance in low-resource settings.


<details>
  <summary>Details</summary>
Motivation: EEG and other biosignals are high-dimensional and noisy, requiring extensive preprocessing. Foundation models offer promise but face challenges in biosignal applications, necessitating alternative representation learning approaches.

Method: BioCodec uses neural codec-inspired representation learning to capture low-level signal characteristics as discrete tokens. It's pre-trained on thousands of EEG hours and can handle multiple biosignal types including EEG and EMG.

Result: BioCodec shows strong performance across clinical diagnostics, sleep physiology, speech decoding, and motor imagery tasks, particularly in low-resource settings. It performs competitively with state-of-the-art models and demonstrates spatial coherence in codebook embeddings.

Conclusion: BioCodec provides a versatile biosignal tokenization solution that effectively handles noisy, high-dimensional data and transfers well to various downstream applications, offering a practical alternative to traditional preprocessing-heavy approaches.
...
### [87] [AdaPM: a Partial Momentum Algorithm for LLM Training](https://arxiv.org/abs/2510.09103)
*Yimu Zhang,Yuanshi Liu,Cong Fang*

Main category: cs.LG

TL;DR: AdaPM is an adaptive training strategy that uses partial momentum to create a memory-efficient optimizer, reducing momentum memory by over 90% while maintaining performance across various language model training tasks.


<details>
  <summary>Details</summary>
Motivation: Large language model training faces memory challenges from storing momentum, which is widely used for acceleration but consumes significant memory resources.

Method: AdaPM employs non-uniform momentum design where full momentum is not needed for most blocks, enhanced with bias correction to mitigate performance loss from partial momentum.

Result: Reduces momentum memory by over 90%, maintains efficiency and performance for models from 60M to 1.5B parameters, and saves over 30% GPU hours for GPT-2 1.5B pretraining when combined with second-order statistic optimization.

Conclusion: AdaPM provides an effective memory-efficient optimization approach that significantly reduces memory usage while preserving training performance across various language model training scenarios.
...
### [88] [MemLoss: Enhancing Adversarial Training with Recycling Adversarial Examples](https://arxiv.org/abs/2510.09105)
*Soroush Mahdi,Maryam Amirmazlaghani,Saeed Saravani,Zahra Dehghanian*

Main category: cs.LG

TL;DR: MemLoss improves adversarial training by reusing previously generated adversarial examples across epochs to enhance model robustness and accuracy without sacrificing clean data performance.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between natural accuracy and adversarial robustness in machine learning models, seeking a balanced improvement in both aspects.

Method: Proposes MemLoss approach that leverages 'Memory Adversarial Examples' - previously generated adversarial examples - across training epochs to enhance model training.

Result: Experimental results on multiple datasets including CIFAR-10 show better accuracy compared to existing adversarial training methods while maintaining strong robustness against attacks.

Conclusion: MemLoss provides an effective approach for adversarial training that achieves balanced improvements in both natural accuracy and adversarial robustness.
...
### [89] [On the Fairness of Privacy Protection: Measuring and Mitigating the Disparity of Group Privacy Risks for Differentially Private Machine Learning](https://arxiv.org/abs/2510.09114)
*Zhi Yang,Changwu Huang,Ke Tang,Xin Yao*

Main category: cs.LG

TL;DR: This paper addresses the underexplored issue of fairness in privacy protection across groups in differentially private machine learning (DPML). It introduces a novel membership inference game for efficient auditing of worst-case privacy risks and proposes an enhanced DP-SGD algorithm with adaptive group-specific gradient clipping to reduce disparity in group privacy risks.


<details>
  <summary>Details</summary>
Motivation: Existing methods for assessing group privacy risks rely on average-case privacy risks of data records, which may underestimate group privacy risks and disparities. Current worst-case privacy risk assessment methods are time-consuming and impractical. There's a need for more efficient and reliable methods to measure and address fairness in privacy protection across different groups.

Method: 1) Introduced a novel membership inference game to efficiently audit approximate worst-case privacy risks of data records. 2) Enhanced the standard DP-SGD algorithm with an adaptive group-specific gradient clipping strategy inspired by differential privacy auditing canaries.

Result: Experimental results show that the proposed method provides more stringent measurement of group privacy risks and reliable assessment of disparity across groups. The enhanced DP-SGD algorithm effectively reduces disparity in group privacy risks, thereby improving fairness in privacy protection for DPML.

Conclusion: The paper successfully addresses the gap in fairness-aware privacy protection by providing efficient tools for worst-case privacy risk assessment and developing practical algorithms that enhance fairness in differentially private machine learning through adaptive group-specific privacy protection mechanisms.
...
### [90] [Regret Bounds for Adversarial Contextual Bandits with General Function Approximation and Delayed Feedback](https://arxiv.org/abs/2510.09127)
*Orin Levy,Liad Erez,Alon Cohen,Yishay Mansour*

Main category: cs.LG

TL;DR: The paper presents regret minimization algorithms for contextual multi-armed bandits with adversarial delayed feedback, achieving optimal bounds in finite policy settings and novel bounds in function approximation settings.


<details>
  <summary>Details</summary>
Motivation: To address the challenge of delayed feedback in contextual multi-armed bandit problems, particularly when delays are chosen adversarially, which is common in real-world applications like online advertising and recommendation systems.

Method: Two approaches: 1) For finite policy classes, direct access to policy class; 2) For general function approximation, using online least-square regression oracle with FIFO order and novel stability analysis of Hedge-based aggregating forecaster.

Result: Achieved expected regret bounds: O(√(KT log|Π|) + √(D log|Π|)) for finite policy classes, and O(√(KT R_T(O)) + √(d_max D β)) for function approximation, with β ≤ log|F| for finite function classes.

Conclusion: The proposed algorithms provide optimal regret bounds for contextual bandits with adversarial delayed feedback, with the function approximation approach achieving bounds close to the established lower bound.
...
### [91] [Score-Based Density Estimation from Pairwise Comparisons](https://arxiv.org/abs/2510.09146)
*Petrus Mikkola,Luigi Acerbi,Arto Klami*

Main category: cs.LG

TL;DR: The paper presents a method for estimating probability densities from pairwise comparison data by relating the target density to a tempered winner density through score-matching and de-tempering techniques.


<details>
  <summary>Details</summary>
Motivation: The research is motivated by expert knowledge elicitation and learning from human feedback, where direct density estimation is challenging but pairwise comparisons are more accessible.

Method: The method relates the unobserved target density to a tempered winner density, learns the winner's score via score-matching, and estimates the target by de-tempering the estimated winner density's score. It uses analytical formulas for the tempering field and proposes an estimator under the Bradley-Terry model, employing diffusion models trained on tempered samples generated via score-scaled annealed Langevin dynamics.

Result: The approach enables learning complex multivariate belief densities of simulated experts from only hundreds to thousands of pairwise comparisons, demonstrating efficient density estimation from limited comparison data.

Conclusion: The paper establishes a theoretical connection between target and winner densities through tempering fields and provides a practical framework for estimating complex densities from pairwise comparison data, which has applications in expert knowledge elicitation and human feedback learning.
...
### [92] [Logits Replay + MoClip: Stabilized, Low-Cost Post-Training with Minimal Forgetting](https://arxiv.org/abs/2510.09152)
*Suming Qiu,Jing Li,Zhicheng Zhou,Junjie Huang,Linyuan Qiu,Zhijie Sun*

Main category: cs.LG

TL;DR: Logits Replay + MoClip is a two-stage framework that compresses supervision in logit space and stabilizes optimization, enabling efficient domain adaptation of LLMs without sacrificing general capabilities.


<details>
  <summary>Details</summary>
Motivation: To address the trade-off between improving specialized domain performance and maintaining general capabilities in LLMs, while reducing computational costs of existing solutions.

Method: Two-stage approach: Stage 0 records dynamic Top-K token subsets covering probability threshold (including gold label); Stage 1 replays compact subsets to compute exact renormalized losses. MoClip optimizer caps gradient-momentum rotation and applies arctan2-based rescaling.

Result: Improves domain performance on Communication Technology and NL2SQL tasks while mitigating forgetting on general benchmarks (MMLU, BBH, GPQA, MATH), with over 40% training cost reduction.
...
### [93] [Agentic-KGR: Co-evolutionary Knowledge Graph Construction through Multi-Agent Reinforcement Learning](https://arxiv.org/abs/2510.09156)
*Jing Li,Zhijie Sun,Zhicheng Zhou,Suming Qiu,Junjie Huang,Haijia Sun,Linyuan Qiu*

Main category: cs.LG

TL;DR: Agentic-KGR is a framework that enables co-evolution between LLMs and knowledge graphs through multi-round reinforcement learning, addressing limitations of static knowledge bases with dynamic schema expansion, retrieval-augmented memory, and learnable prompt compression.


<details>
  <summary>Details</summary>
Motivation: Current knowledge-enhanced LLMs rely on static knowledge bases that suffer from coverage gaps and temporal obsolescence, limiting effectiveness in dynamic information environments.

Method: Uses multi-round RL with three innovations: dynamic schema expansion to extend graph ontologies, retrieval-augmented memory for synergistic co-evolution, and learnable multi-scale prompt compression to reduce computational complexity.

Result: Achieves substantial improvements over supervised baselines and single-round RL in knowledge extraction tasks. When integrated with GraphRAG, shows superior performance in downstream QA tasks with significant gains in accuracy and knowledge coverage.

Conclusion: The framework successfully enables co-evolution between LLMs and knowledge graphs, overcoming limitations of static knowledge bases and demonstrating enhanced performance in dynamic information environments.
...
### [94] [Cross-Representation Benchmarking in Time-Series Electronic Health Records for Clinical Outcome Prediction](https://arxiv.org/abs/2510.09159)
*Tianyi Chen,Mingcheng Zhu,Zhiyao Luo,Tingting Zhu*

Main category: cs.LG

TL;DR: This paper presents the first systematic benchmark comparing EHR representation methods (multivariate time-series, event streams, textual event streams) across ICU and longitudinal care settings, finding that event stream models consistently perform best and feature selection strategies must be adapted to clinical context.


<details>
  <summary>Details</summary>
Motivation: To address inconsistent evaluation practices in EHR representation methods and provide standardized comparison of different approaches for clinical predictions.

Method: Created systematic benchmark standardizing data curation and evaluation across MIMIC-IV (ICU tasks) and EHRSHOT (longitudinal care) datasets. Evaluated Transformers, MLP, LSTMs, Retain for time-series; CLMBR and count-based models for event streams; 8-20B LLMs for textual streams. Analyzed impact of feature pruning based on data missingness.

Result: Event stream models consistently delivered strongest performance. Pre-trained models like CLMBR were highly sample-efficient in few-shot settings, while simpler count-based models could be competitive with sufficient data. Feature selection strategies must be adapted: pruning sparse features improves ICU predictions, while retaining them is critical for longitudinal tasks.

Conclusion: The unified benchmark provides practical guidance for selecting EHR representations based on clinical context and data regime, with event stream models emerging as the most effective approach overall.
...
### [95] [Efficient Resource-Constrained Training of Vision Transformers via Subspace Optimization](https://arxiv.org/abs/2510.09160)
*Le-Trung Nguyen,Enzo Tartaglione,Van-Tam Nguyen*

Main category: cs.LG

TL;DR: WASI is a subspace-based training method for transformers that reduces memory usage by 62x and computational cost by 2x while maintaining accuracy comparable to vanilla training, enabling efficient on-device learning.


<details>
  <summary>Details</summary>
Motivation: Address energy consumption and data privacy concerns in AI by enabling on-device learning, overcoming the memory bottleneck of large transformer models through subspace-based training.

Method: Weight-Activation Subspace Iteration (WASI) restricts training to a fixed subspace containing the model's essential information, mitigating backpropagation memory requirements and improving inference efficiency.

Result: WASI maintains comparable accuracy to vanilla training while reducing memory usage by up to 62x, computational cost (FLOPs) by up to 2x, and achieves 1.5x faster training/inference on Raspberry Pi 5.

Conclusion: Subspace-based training effectively enables efficient on-device transformer training, addressing energy and privacy concerns while maintaining model performance.
...
### [96] [Robustness and Regularization in Hierarchical Re-Basin](https://arxiv.org/abs/2510.09174)
*Benedikt Franke,Florian Heinrich,Markus Lange,Arne Raul*

Main category: cs.LG

TL;DR: This paper analyzes Git Re-Basin model merging and proposes a hierarchical merging scheme that outperforms standard MergeMany, finding it induces robustness but causes larger performance drops than originally reported.


<details>
  <summary>Details</summary>
Motivation: To investigate Git Re-Basin model merging more thoroughly and improve upon the standard merging algorithms.

Method: Proposed a hierarchical model merging scheme and conducted experiments comparing it with the standard MergeMany algorithm.

Result: The hierarchical merging significantly outperforms MergeMany, induces adversarial and perturbation robustness that strengthens with more models, but causes larger performance drops than originally reported.

Conclusion: While Git Re-Basin's hierarchical merging improves robustness, it comes with more significant performance degradation than previously indicated.
...
### [97] [Beyond Pairwise Connections: Extracting High-Order Functional Brain Network Structures under Global Constraints](https://arxiv.org/abs/2510.09175)
*Ling Zhan,Junjie Huang,Xiaoyao Yu,Wenyu Chen,Tao Jia*

Main category: cs.LG

TL;DR: The paper proposes GCM, a Global Constraints oriented Multi-resolution framework for learning high-order functional brain network structures, addressing limitations of local pairwise interactions and heuristic hypergraph approaches through 4 global constraints across different modeling resolutions.


<details>
  <summary>Details</summary>
Motivation: Current functional brain network modeling relies on local pairwise interactions which cannot capture high-order dependencies, while existing hypergraph approaches suffer from computational burden and heuristic nature, preventing end-to-end learning from data distributions.

Method: Proposed GCM framework extracts high-order FBN structures under 4 global constraints (signal synchronization, subject identity, expected edge numbers, data labels) for 4 modeling resolution levels (sample/subject/group/project).

Result: GCM achieves up to 30.6% improvement in relative accuracy and 96.3% reduction in computational time across 5 datasets and 2 task settings, outperforming 9 baselines and 10 state-of-the-art methods.

Conclusion: The work provides a novel perspective on FBN structure learning with demonstrated interpretability and serves as a foundation for interdisciplinary applications in cognitive neuroscience.
...
### [98] [RepDL: Bit-level Reproducible Deep Learning Training and Inference](https://arxiv.org/abs/2510.09180)
*Peichen Xie,Xian Zhang,Shuo Chen*

Main category: cs.LG

TL;DR: RepDL is an open-source library that ensures deterministic and bitwise-reproducible deep learning training and inference by addressing floating-point computation inconsistencies.


<details>
  <summary>Details</summary>
Motivation: Non-determinism and non-reproducibility in deep learning cause inconsistent results across runs and platforms, primarily due to random number generation and floating-point computation issues.
...
Result: The library successfully ensures deterministic deep learning training and inference across diverse computing environments.

Conclusion: RepDL provides a solution to floating-point computation inconsistencies, enabling reproducible deep learning results.
...
### [99] [On the Implicit Adversariality of Catastrophic Forgetting in Deep Continual Learning](https://arxiv.org/abs/2510.09181)
*Ze Peng,Jian Zhang,Jintao Guo,Lei Qi,Yang Gao,Yinghuan Shi*

Main category: cs.LG

TL;DR: This paper reveals that catastrophic forgetting in continual learning occurs because new-task training acts as an adversarial attack on old-task knowledge, with gradients automatically aligning with sharp directions of the old-task loss landscape due to training's low-rank bias.


<details>
  <summary>Details</summary>
Motivation: To demystify the underlying cause of catastrophic forgetting in deep networks and understand why new-task training rapidly increases old-task loss.

Method: Theoretical analysis showing how training's low-rank bias causes gradient alignment, and proposing backGP method to address alignment from backward propagation that existing gradient projection methods miss.

Result: BackGP reduces forgetting by 10.8% and improves accuracy by 12.7% on average over existing gradient projection methods.

Conclusion: Catastrophic forgetting stems from adversarial gradient alignment due to low-rank bias, and addressing both forward and backward propagation alignment is crucial for effective continual learning.
...
### [100] [Multimodal Prompt Optimization: Why Not Leverage Multiple Modalities for MLLMs](https://arxiv.org/abs/2510.09201)
*Yumin Choi,Dongki Kim,Jinheon Baek,Sung Ju Hwang*

Main category: cs.LG

TL;DR: The paper introduces multimodal prompt optimization, extending prompt optimization beyond text to include images, videos, and other modalities. It proposes MPO, a framework that jointly optimizes multimodal prompts using alignment-preserving updates and Bayesian-based selection.


<details>
  <summary>Details</summary>
Motivation: Current prompt optimization methods are limited to text-only approaches, which restricts the full potential of multimodal large language models (MLLMs) that can handle various modalities beyond text.
...
Result: Extensive experiments across diverse modalities (images, videos, molecules) show that MPO outperforms leading text-only optimization methods.

Conclusion: Multimodal prompt optimization is a crucial step to realizing the potential of MLLMs, and MPO effectively addresses this need by extending prompt optimization to the multimodal space.
...
### [101] [FM-IRL: Flow-Matching for Reward Modeling and Policy Regularization in Reinforcement Learning](https://arxiv.org/abs/2510.09222)
*Zhenglin Wan,Jingxuan Wu,Xingrui Yu,Chubin Zhang,Mingcong Lei,Bo An,Ivor Tsang*

Main category: cs.LG

TL;DR: The paper proposes a student-teacher framework where a simple MLP student policy explores online via RL with rewards from a teacher Flow Matching model, overcoming FM's limitations in online interaction while leveraging its distribution modeling capabilities.


<details>
  <summary>Details</summary>
Motivation: Flow Matching policies have strong behavioral cloning performance but lack environmental interaction and exploration, leading to poor generalization in unseen scenarios. Online optimization of FM policies is challenging due to gradient instability and high inference costs.

Method: Use a student MLP policy for online exploration and RL updates, guided by a reward model from a teacher FM model. The teacher FM model provides expert distribution information and regularizes the student policy to stabilize learning.

Result: Extensive experiments show significant improvements in learning efficiency, generalization, and robustness, particularly when learning from suboptimal expert data.

Conclusion: The student-teacher framework successfully combines the expressiveness of FM models with efficient online exploration, overcoming FM's limitations while maintaining strong performance.
...
### [102] [Prime Implicant Explanations for Reaction Feasibility Prediction](https://arxiv.org/abs/2510.09226)
*Klaus Weinbauer,Tieu-Long Phan,Peter F. Stadler,Thomas Gärtner,Sagar Malhotra*

Main category: cs.LG

TL;DR: A novel prime implicant explanation method for chemical reaction feasibility prediction models that improves interpretability by identifying essential molecular attributes.


<details>
  <summary>Details</summary>
Motivation: Current machine learning models for predicting chemical reaction feasibility lack transparency and interpretability, making it difficult to understand their decision-making process.

Method: Developed a novel formulation of prime implicant explanations (minimally sufficient reasons) specifically for chemical reaction prediction, with an algorithm to compute these explanations in small-scale tasks.

Result: Preliminary experiments show that prime implicant explanations conservatively capture ground truth explanations - they often include redundant bonds and atoms but consistently identify the essential molecular attributes needed for reaction feasibility prediction.

Conclusion: The proposed prime implicant explanation method successfully identifies the key molecular features that drive reaction feasibility predictions, providing interpretability while maintaining conservative accuracy.
...
### [103] [Incentivizing Time-Aware Fairness in Data Sharing](https://arxiv.org/abs/2510.09240)
*Jiangwei Chen,Kieu Thao Nguyen Pham,Rachael Hwee Ling Sim,Arun Verma,Zhaoxuan Wu,Chuan-Sheng Foo,Bryan Kian Hsiang Low*

Main category: cs.LG

TL;DR: The paper proposes a time-aware data sharing framework that rewards early contributors in collaborative machine learning, addressing the issue of parties joining at different times rather than simultaneously.


<details>
  <summary>Details</summary>
Motivation: Current collaborative ML frameworks assume all parties join simultaneously, but real-world scenarios involve staggered participation due to data cleaning time, legal barriers, or unawareness. Early joiners incur higher risk and encourage others to contribute, so they deserve higher rewards.

Method: Developed a fair and time-aware data sharing framework with novel time-aware incentives. Created methods for determining reward values that satisfy these incentives and generate model rewards to realize the determined values.

Result: Empirical demonstration on synthetic and real-world datasets shows the proposed methods satisfy the desired properties of time-aware incentives in collaborative data sharing.

Conclusion: The time-aware framework successfully addresses the real-world challenge of staggered participation in collaborative ML by providing appropriate incentives for early contributors, ensuring fairness and individual rationality in data sharing scenarios.
...
### [104] [A PCA-based Data Prediction Method](https://arxiv.org/abs/2510.09246)
...
<details>
  <summary>Details</summary>
Motivation: The frequent problem of missing data in data science requires effective imputation methods to handle incomplete datasets.

Method: Uses distance between shifted linear subspaces representing existing data and candidate sets, with the existing data represented by subspace spanned by first principal components. Solutions provided for Euclidean metric.

Result: The paper presents a mathematical framework for missing data imputation that integrates subspace analysis with machine learning approaches.

Conclusion: The proposed method offers a novel approach to missing data imputation by leveraging subspace distance concepts and principal component analysis.
...
### [105] [Mitigating Model Drift in Developing Economies Using Synthetic Data and Outliers](https://arxiv.org/abs/2510.09294)
...
<details>
  <summary>Details</summary>
Motivation: Machine learning models in finance are highly vulnerable to model drift due to shifting data distributions, particularly in developing economies like Tajikistan, Uzbekistan, Kazakhstan, and Azerbaijan where frequent macroeconomic shocks destabilize financial data. This is one of the first studies to examine drift mitigation methods specifically for these regions.

Method: The study investigates the use of synthetic outliers as a drift mitigation approach and introduces a two-level evaluation framework that measures both performance degradation extent and shock severity. Experiments are conducted on macroeconomic tabular datasets.

Result: Experiments show that adding a small proportion of synthetic outliers generally improves model stability compared to baseline models, though the optimal amount varies by dataset and model type.

Conclusion: Synthetic outliers can be an effective approach for improving ML model stability against unforeseen macroeconomic shocks in developing economies, with the effectiveness depending on dataset and model characteristics.
...
### [106] [Large Language Model Prompt Datasets: An In-depth Analysis and Insights](https://arxiv.org/abs/2510.09316)
*Yuanming Zhang,Yan Lin,Arijit Khan,Huaiyu Wan*

Main category: cs.LG

TL;DR: This paper compiles the first extensive collection of prompt datasets from various sources, analyzes their characteristics, and proposes a prompt optimization method using syntactic embeddings to improve LLM outputs.


<details>
  <summary>Details</summary>
Motivation: With the growing deployment of LLMs, diverse prompt datasets are emerging but lack systematic analysis. The authors aim to understand prompt construction patterns across different categories and improve prompt engineering through optimization.

Method: Compiled extensive prompt datasets from various channels, selected representative datasets for systematic analysis, and proposed a prompt optimization approach using syntactic embeddings of part-of-speech and dependency structures to guide LLMs in rewriting prompts toward a centroid representation.

Result: The analysis revealed commonalities and differences in prompt construction across categories, distinguishing them from other text corpora. The proposed optimization method improved the meaningfulness of model outputs by guiding prompt rewriting toward identified centroid representations.

Conclusion: The work provides comprehensive prompt dataset resources and demonstrates that syntactic structure-based optimization can enhance prompt effectiveness, contributing to better LLM utilization and prompt engineering practices.
...
### [107] [Residual-Informed Learning of Solutions to Algebraic Loops](https://arxiv.org/abs/2510.09317)
*Felix Brandt,Andreas Heuermann,Philip Hannebohm,Bernhard Bachmann*

Main category: cs.LG

TL;DR: A residual-informed machine learning approach replaces algebraic loops in Modelica models with neural network surrogates, achieving 60% faster simulations while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: To improve simulation efficiency by replacing computationally expensive algebraic loops in equation-based Modelica models with faster neural network surrogates.

Method: Uses feedforward neural networks trained with algebraic loop residuals directly in the loss function, eliminating need for supervised datasets and resolving ambiguous solution issues.
...
### [108] [Rate optimal learning of equilibria from data](https://arxiv.org/abs/2510.09325)
*Till Freihaut,Luca Viano,Emanuele Nevali,Volkan Cevher,Matthieu Geist,Giorgia Ramponi*

Main category: cs.LG

TL;DR: This paper addresses theoretical gaps in Multi-Agent Imitation Learning (MAIL) by proving statistical lower bounds for non-interactive settings and introducing an interactive algorithm with near-optimal sample complexity.


<details>
  <summary>Details</summary>
Motivation: To close theoretical gaps in MAIL by characterizing the limits of non-interactive approaches and developing more efficient interactive algorithms with improved sample complexity.

Method: Proved statistical lower bounds for non-interactive MAIL, identified the all-policy deviation concentrability coefficient as key complexity measure, and introduced MAIL-WARM algorithm combining reward-free RL with interactive MAIL.

Result: Showed Behavior Cloning is rate-optimal in non-interactive settings, improved sample complexity from O(ε⁻⁸) to O(ε⁻²) for interactive MAIL, and provided numerical validation in grid world environments.

Conclusion: The paper establishes fundamental complexity measures for MAIL, proves optimality of Behavior Cloning in non-interactive settings, and introduces a near-optimal interactive algorithm that significantly improves sample efficiency.
...
### [109] [Safety Game: Balancing Safe and Informative Conversations with Blackbox Agentic AI using LP Solvers](https://arxiv.org/abs/2510.09330)
*Tuan Nguyen,Long Tran-Thanh*

Main category: cs.LG

TL;DR: A black-box framework for LLM safety alignment using game theory, enabling safety enforcement without model retraining or internal access.
...
Method: Formulates safety-helpfulness trade-off as a two-player zero-sum game, using linear programming solver at inference time to compute equilibrium strategies.

Result: Demonstrates feasibility of black-box safety alignment that balances safety and helpfulness effectively.

Conclusion: Provides scalable, accessible pathway for stakeholders to enforce safety across evolving LLM ecosystems without requiring model retraining or internal access.
...
### [110] [Efficient Bayesian Inference from Noisy Pairwise Comparisons](https://arxiv.org/abs/2510.09333)
...
Method: Introduces BBQ, a Bayesian Bradley-Terry variant that models rater quality explicitly, downweights unreliable participants, and uses Expectation-Maximization algorithm for guaranteed monotonic likelihood convergence.
...
### [111] [Deep Learning to Identify the Spatio-Temporal Cascading Effects of Train Delays in a High-Density Network](https://arxiv.org/abs/2510.09350)
*Vu Duc Anh Nguyen,Ziyue Li*

Main category: cs.LG

TL;DR: The paper proposes a novel XGeoAI framework for live, explainable, multi-step train delay forecasting using a two-stage autoregressive Graph Attention Network (GAT) model trained on Dutch railway data.


<details>
  <summary>Details</summary>
Motivation: Railway operational efficiency is undermined by cascading train delays, and existing GNN approaches lack multi-step autoregressive forecasting with live interpretable explanations needed for decision support.

Method: A two-stage autoregressive Graph Attention Network (GAT) model representing the railway system as a spatio-temporal graph of operational events, enriched with platform and station congestion features, evaluated using sequential k-step-ahead forecasting protocol.
...
Conclusion: The proposed framework provides a viable approach for live, explainable multi-step train delay forecasting with practical advantages for decision support systems despite challenges in pure error metrics.
...
### [112] [The Potential of Second-Order Optimization for LLMs: A Study with Full Gauss-Newton](https://arxiv.org/abs/2510.09378)
*Natalie Abreu,Nikhil Vyas,Sham Kakade,Depen Morwani*

Main category: cs.LG

TL;DR: Full Gauss-Newton preconditioning achieves 5.4x training iteration reduction vs baselines, with layerwise approximation nearly matching full method performance.


<details>
  <summary>Details</summary>
Motivation: To determine how much performance is lost by computationally-efficient approximations in LLM pretraining compared to full second-order methods.

Method: Applied full Gauss-Newton preconditioning to transformer models up to 150M parameters, comparing against baselines like SOAP and Muon, and testing layerwise GN preconditioner.

Result: Full GN updates yield substantial gains with 5.4x reduction in training iterations; layerwise GN preconditioner nearly matches full GN performance.

Conclusion: GN approximation is highly effective for preconditioning, layerwise Hessian structure contains sufficient information for most gains, and significant performance gap exists between current methods and idealized layerwise oracle.
...
### [113] [CHUCKLE -- When Humans Teach AI To Learn Emotions The Easy Way](https://arxiv.org/abs/2510.09382)
...
<details>
  <summary>Details</summary>
Motivation: Existing curriculum learning approaches for emotion recognition rely on heuristic or model-based difficulty definitions, ignoring human perception difficulty which is crucial for subjective tasks like emotion recognition.
...
Result: Increases relative mean accuracy by 6.56% for LSTMs and 1.61% for Transformers over non-curriculum baselines, while reducing the number of gradient updates.
...
### [114] [HINT: Helping Ineffective Rollouts Navigate Towards Effectiveness](https://arxiv.org/abs/2510.09388)
...
<details>
  <summary>Details</summary>
Motivation: Existing RL methods like GRPO often fail when task difficulty exceeds model capacity, leading to reward sparsity and inefficient training. Prior approaches using off-policy data (SFT mixing or hints) often misguide policy updates due to low training affinity.

Method: Introduces Affinity metric to monitor exploration efficiency and training stability. Proposes HINT framework that supplies heuristic hints to guide models toward discovering solutions autonomously, preserving reasoning capabilities.
...
Conclusion: HINT effectively addresses the low training affinity problem in RL for LLMs, enabling more efficient and stable training while maintaining autonomous reasoning capabilities.
...
### [115] [Cross-Receiver Generalization for RF Fingerprint Identification via Feature Disentanglement and Adversarial Training](https://arxiv.org/abs/2510.09405)
*Yuhao Pan,Xiucheng Wang,Nan Cheng,Wenchao Xu*

Main category: cs.LG

TL;DR: The paper proposes a robust radio frequency fingerprint identification (RFFI) framework that addresses cross-receiver variability using adversarial training and style transfer to disentangle transmitter and receiver features, achieving up to 10% improvement in accuracy.


<details>
  <summary>Details</summary>
Motivation: Current RFFI models overfit to receiver-specific patterns, causing substantial performance degradation when receivers are replaced during deployment. Receiver-induced feature shifts remain largely unaddressed despite channel equalization.

Method: The framework integrates adversarial training and style transfer to explicitly disentangle transmitter and receiver features, enforcing domain-invariant representation learning to isolate genuine hardware signatures from receiver artifacts.

Result: Extensive experiments on multi-receiver datasets show the approach consistently outperforms state-of-the-art baselines, achieving up to 10% improvement in average accuracy across diverse receiver settings.

Conclusion: The proposed method effectively addresses cross-receiver variability in RFFI systems, ensuring robustness against receiver changes by isolating transmitter-specific hardware signatures from receiver-induced biases.
...
### [116] [What Do Temporal Graph Learning Models Learn?](https://arxiv.org/abs/2510.09416)
*Abigail J. Hayes,Tobias Schumacher,Markus Strohmaier*

Main category: cs.LG

TL;DR: This paper systematically evaluates 7 temporal graph learning models on their ability to capture 8 fundamental graph attributes, revealing that models perform well on some attributes but fail on others, exposing important limitations in current approaches.


<details>
  <summary>Details</summary>
Motivation: Recent concerns about unreliable benchmark results and the surprising competitiveness of simple heuristics raise questions about what properties temporal graph learning models actually use for predictions, motivating a systematic evaluation of model capabilities.

Method: Systematically evaluated 7 temporal graph learning models on their ability to capture 8 fundamental attributes related to temporal graph link structure, using both synthetic and real-world datasets to analyze attribute learning.

Result: Findings reveal a mixed picture - models capture some attributes well but fail to reproduce others, exposing important limitations in current temporal graph learning approaches.

Conclusion: The results provide practical insights for applying temporal graph learning models and motivate more interpretability-driven evaluations in temporal graph learning research to better understand model capabilities and limitations.
...
### [117] [Weight Initialization and Variance Dynamics in Deep Neural Networks and Large Language Models](https://arxiv.org/abs/2510.09423)
*Yankun Han*

Main category: cs.LG

TL;DR: This paper analyzes weight initialization effects across ReLU MLPs and GPT-2 transformers, identifying optimal initialization ranges and showing Kaiming initialization outperforms Xavier for ReLU networks. It also tracks transformer weight variance evolution during pretraining.


<details>
  <summary>Details</summary>
Motivation: To provide a theory-grounded and empirically validated study of weight initialization across different neural network architectures, connecting classic initialization principles with modern transformer behavior.

Method: Conducted logarithmic sweep of initial standard deviations, compared Kaiming vs Xavier initialization for ReLU networks, and tracked layerwise Q/K/V weight variance evolution in a 12-layer GPT-2-style transformer during pretraining.

Result: Found stability band with standard deviations between 1e-2 and 1e-1, Kaiming initialization converges faster and more stably than Xavier under ReLU, and observed depth-dependent equilibration in transformers where shallow layers expand rapidly while deeper layers change gradually.

Conclusion: The results connect classic initialization principles with modern transformer behavior and provide simple, practical recipes for robust training across different neural network architectures.
...
### [118] [Bandits with Single-Peaked Preferences and Limited Resources](https://arxiv.org/abs/2510.09425)
*Gur Keinan,Rotem Torkan,Omer Ben-Porat*

Main category: cs.LG

TL;DR: The paper studies online stochastic matching with budget constraints, focusing on single-peaked preferences to overcome NP-hardness. It develops efficient offline and online algorithms with regret bounds of O(UKT^{2/3}) and O(U√TK) respectively.


<details>
  <summary>Details</summary>
Motivation: Without structural assumptions, computing optimal matching in online stochastic problems is NP-hard, making online learning computationally infeasible. The paper aims to overcome this barrier using single-peaked preferences from social choice theory.

Method: The approach uses single-peaked preferences where users' preferences are unimodal with respect to a common arm order. It develops an efficient offline budgeted matching algorithm and leverages it into online algorithms using PQ tree-based order approximation and UCB-like methods.

Result: The algorithm achieves a regret bound of O(UKT^{2/3}) for the general case and O(U√TK) when the single-peaked structure is known, both with efficient computational complexity.

Conclusion: Single-peaked preferences provide a computationally tractable framework for online stochastic matching problems, enabling efficient algorithms with strong regret guarantees despite the general problem's NP-hardness.
...
### [119] [Cross-attention Secretly Performs Orthogonal Alignment in Recommendation Models](https://arxiv.org/abs/2510.09435)
...
TL;DR: Cross-attention in CDSR can function as both residual alignment and orthogonal alignment, where orthogonal alignment discovers novel information not present in the query input. This phenomenon naturally emerges without constraints and improves model performance and scaling efficiency.


<details>
  <summary>Details</summary>
Motivation: To better understand the underlying mechanisms of cross-attention in cross-domain sequential recommendation, particularly moving beyond the prevailing view of residual alignment to explore orthogonal alignment phenomena.

Method: Conducted over 300 experiments analyzing cross-attention mechanisms in CDSR models, examining when query inputs and outputs become orthogonal and how this affects performance.

Result: Orthogonal alignment naturally emerges without explicit constraints and improves model performance. Models with cross-attention modules outperform parameter-matched baselines, achieving superior accuracy-per-model parameter.

Conclusion: Orthogonal alignment and residual alignment can coexist in recommendation models, with orthogonal alignment improving scaling laws and offering new directions for parameter-efficient scaling in multi-modal research.
...
### [120] [On Uniformly Scaling Flows: A Density-Aligned Approach to Deep One-Class Classification](https://arxiv.org/abs/2510.09452)
*Faried Abu Zaid,Tim Katzke,Emmanuel Müller,Daniel Neider*

Main category: cs.LG

TL;DR: This paper shows that uniformly scaling flows (USFs) connect deep one-class classification and density estimation approaches in unsupervised anomaly detection, proving that training USFs via maximum-likelihood reduces to a Deep SVDD objective with unique regularization.


<details>
  <summary>Details</summary>
Motivation: To bridge the gap between two major paradigms in unsupervised anomaly detection: deep one-class classification (like Deep SVDD) and density estimation (via normalizing flows), showing their theoretical connection through uniformly scaling flows.

Method: Theoretical analysis proving that training uniformly scaling flows (USFs) via maximum-likelihood reduces to a Deep SVDD objective with inherent regularization that prevents representational collapse. Also demonstrates how USFs can replace non-USFs in existing architectures.

Result: USFs induce tighter alignment between negative log-likelihood and latent norm than either Deep SVDD or non-USFs. Empirical results show consistent performance gains and improved training stability across multiple benchmarks for both image-level and pixel-level anomaly detection.

Conclusion: USFs provide a theoretical bridge between two major anomaly detection paradigms, unifying deep one-class methods and density estimation approaches, while offering practical performance improvements as drop-in replacements for non-USFs in modern architectures.
...
### [121] [Adaptive Attacks on Trusted Monitors Subvert AI Control Protocols](https://arxiv.org/abs/2510.09462)
*Mikhail Terekhov,Alexander Panfilov,Daniil Dzenhaliou,Caglar Gulcehre,Maksym Andriushchenko,Ameya Prabhu,Jonas Geiping*

Main category: cs.LG

TL;DR: Adaptive attacks on AI control protocols reveal vulnerabilities in LLM monitors, showing that untrusted models can evade detection using prompt injection attacks, making current protocols ineffective.


<details>
  <summary>Details</summary>
Motivation: To study how untrusted LLM agents can bypass AI control protocols by exploiting knowledge of the monitor model and deployment context, highlighting a critical security gap.

Method: Instantiate adaptive attacks where attackers embed publicly known or zero-shot prompt injections in model outputs, testing against diverse monitors on AI control benchmarks.

Result: Frontier models consistently evade monitors and complete malicious tasks, with the Defer-to-Resample protocol backfiring by amplifying prompt injections.

Conclusion: Adaptive attacks on monitor models are a major blind spot in current AI control protocols and should be included in future evaluations.
...
### [122] [Geodesic Calculus on Latent Spaces](https://arxiv.org/abs/2510.09468)
...
<details>
  <summary>Details</summary>
Motivation: To provide geometric analysis tools for latent manifolds of autoencoders that are robust against representation inaccuracies and support different Riemannian geometries.

Method: Learn an approximate projection onto latent manifolds using denoising objective, develop discrete Riemannian calculus tools for geometric operators, and enable computation of geodesics and exponential maps.
...
Conclusion: The proposed approach provides robust geometric analysis tools for latent manifolds that work with different autoencoders and support various Riemannian geometries, enabling effective computation of geometric properties.
...
### [123] [CRPS-LAM: Regional ensemble weather forecasting from matching marginals](https://arxiv.org/abs/2510.09484)
*Erik Larsson,Joel Oskarsson,Tomas Landelius,Fredrik Lindsten*

Main category: cs.LG

TL;DR: CRPS-LAM is a probabilistic regional weather forecasting model that uses CRPS-based training to generate ensemble members efficiently in a single forward pass, achieving 39x faster sampling than diffusion models while maintaining accuracy.


<details>
  <summary>Details</summary>
Motivation: Diffusion-based models for weather prediction show strong performance but are computationally expensive at sampling time, creating a need for more efficient probabilistic forecasting methods.

Method: The model is trained with a CRPS-based objective and generates ensemble members by sampling and injecting a single latent noise vector in a single forward pass, eliminating the iterative sampling process of diffusion models.

Result: CRPS-LAM achieves sampling speeds up to 39 times faster than diffusion-based models while matching their low errors on the MEPS regional dataset and retaining fine-scale forecast details.

Conclusion: CRPS-LAM stands out as an effective approach for probabilistic regional weather forecasting by combining computational efficiency with accurate ensemble generation.
...
### [124] [Locally Optimal Private Sampling: Beyond the Global Minimax](https://arxiv.org/abs/2510.09485)
*Hrad Ghoukasian,Bonwoo Lee,Shahab Asoodeh*

Main category: cs.LG

TL;DR: This paper studies local differential privacy (LDP) sampling, focusing on minimax risk around a fixed distribution P₀ rather than global optimality. The authors characterize exact local minimax risk, show it equals global minimax risk when restricted to P₀'s neighborhood, extend results to functional LDP, and derive closed-form locally optimal samplers that outperform global methods.


<details>
  <summary>Details</summary>
Motivation: Prior work focused on global minimax-optimality across distribution classes, but this paper takes a local perspective to understand sampling performance in neighborhoods around specific distributions, which better models practical scenarios like private sampling with public data.

Method: The authors examine minimax risk around fixed distribution P₀, extend previous pure LDP work to functional LDP framework, prove global optimal functional LDP samplers yield optimal local samplers when constrained to P₀'s neighborhood, and derive closed-form expressions for locally minimax-optimal samplers.

Result: The local minimax risk equals global minimax risk when restricted to distributions near P₀. The derived locally optimal samplers have simple closed-form expressions independent of f-divergence choice, and empirically outperform global minimax samplers in private sampling with public data scenarios.

Conclusion: The local framework provides a more refined understanding of LDP sampling performance around specific distributions, yields practical closed-form optimal samplers, and naturally models real-world scenarios like private sampling with public data where local optimization around known distributions is beneficial.
...
### [125] [Near-Optimal Second-Order Guarantees for Model-Based Adversarial Imitation Learning](https://arxiv.org/abs/2510.09487)
*Shangzhe Li,Dongruo Zhou,Weitong Zhang*

Main category: cs.LG

TL;DR: This paper introduces MB-AIL, a model-based adversarial imitation learning algorithm that achieves horizon-free, second-order sample-complexity guarantees for online learning from offline expert demonstrations without reward access.


<details>
  <summary>Details</summary>
Motivation: Address gaps in understanding the benefits of online interaction and impact of stochasticity in adversarial imitation learning, where current methods have unclear theoretical foundations despite strong empirical performance.

Method: Propose MB-AIL (Model-Based Adversarial Imitation Learning) that leverages general function approximations for both expert data and reward-free online interactions, establishing second-order sample complexity bounds.

Result: MB-AIL achieves minimax-optimal sample complexity for online interaction (up to logarithmic factors) with limited expert demonstrations, with bounds that scale with policy variance and tighten as system approaches determinism.

Conclusion: The theoretical analysis and experiments demonstrate that MB-AIL provides superior sample efficiency compared to existing methods, with practical implementations matching or surpassing current approaches.
...
### [126] [Performance Analysis of Machine Learning Algorithms in Chronic Kidney Disease Prediction](https://arxiv.org/abs/2510.09493)
*Iftekhar Ahmed,Tanzil Ebad Chowdhury,Biggo Bushon Routh,Nafisa Tasmiya,Shadman Sakib,Adil Ahmed Chowdhury*

Main category: cs.LG

TL;DR: This study develops machine learning models for Chronic Kidney Disease (CKD) diagnosis, comparing eight algorithms and finding Random Forest and Logistic Regression achieve 99% accuracy.


<details>
  <summary>Details</summary>
Motivation: With 10% of global population affected by CKD, there's a critical need for effective risk evaluation and monitoring. Machine learning offers quick, precise detection capabilities that can help healthcare practitioners diagnose CKD efficiently.

Method: Used UCL machine learning repository CKD dataset with missing values filled using 'mean-mode' and 'Random sampling method'. Applied eight ML techniques: Random Forest, SVM, Naive Bayes, Logistic Regression, KNN, XGBoost, Decision Tree, and AdaBoost.

Result: Random Forest and Logistic Regression achieved outstanding 99% accuracy, followed by Ada Boost, XGBoost, Naive Bayes, Decision Tree, and SVM. KNN classifier performed worst with 73% accuracy.

Conclusion: Machine learning models, particularly Random Forest and Logistic Regression, show excellent potential for accurate CKD diagnosis, with 99% accuracy demonstrating their effectiveness as computer-aided diagnostic tools.
...
### [127] [Geo-Aware Models for Stream Temperature Prediction across Different Spatial Regions and Scales](https://arxiv.org/abs/2510.09500)
*Shiyuan Luo,Runlong Yu,Shengyu Chen,Yingda Fan,Yiqun Xie,Yanhua Li,Xiaowei Jia*

Main category: cs.LG

TL;DR: Geo-STARS is a geo-aware spatio-temporal modeling framework that uses geographic embeddings to predict stream water temperature across different watersheds and scales, addressing data heterogeneity and limited observation samples in environmental ecosystems.


<details>
  <summary>Details</summary>
Motivation: Existing physics-based and data-driven models fail to generalize across spatial regions and scales due to data heterogeneity and limited observation samples in environmental ecosystems, particularly for stream water temperature prediction.

Method: Proposes geo-aware embedding that leverages geographic information to capture shared patterns across regions and scales, integrated into a gated spatio-temporal graph neural network to learn complex spatial-temporal patterns guided by geographic context.

Result: Geo-STARS demonstrates superior generalization performance across regions and scales compared to state-of-the-art baselines, using real-world datasets spanning 37 years across multiple watersheds along the eastern US coast.
...
### [128] [Automated Evolutionary Optimization for Resource-Efficient Neural Network Training](https://arxiv.org/abs/2510.09566)
*Ilia Revin,Leon Strelkov,Vadim A. Potemkin,Ivan Kireev,Andrey Savchenko*

Main category: cs.LG

TL;DR: PETRA is an AutoML framework that uses evolutionary optimization to automate neural network optimization through pruning, quantization, and loss regularization, achieving significant improvements in model efficiency without performance degradation.


<details>
  <summary>Details</summary>
Motivation: The increasing need for scalable and resource-efficient neural network models due to challenges in distributed computing, compression techniques, and efficient training across various applications.

Method: Developed PETRA framework using evolutionary optimization for model architecture and training strategy, incorporating pruning, quantization, and loss regularization techniques.

Result: Experimental results show significant improvements: 75% reduction in model size, 33% decrease in latency, and 13% increase in throughput without noticeable degradation in target metrics across financial event sequences, image, and time-series benchmarks.

Conclusion: PETRA successfully addresses neural network optimization challenges by providing an automated framework that significantly improves model efficiency while maintaining performance, demonstrating its effectiveness across diverse real-world applications.
...
### [129] [STaTS: Structure-Aware Temporal Sequence Summarization via Statistical Window Merging](https://arxiv.org/abs/2510.09593)
*Disharee Bhowmick,Ranjith Ramanathan,Sathyanarayanan N. Aakur*

Main category: cs.LG

TL;DR: STaTS is an unsupervised framework that compresses time series into compact token sequences by detecting change points across multiple resolutions and summarizing segments, achieving up to 30x compression while preserving temporal dynamics.


<details>
  <summary>Details</summary>
Motivation: Standard time series models treat all time steps equally, leading to inefficiencies and poor robustness. There's a need to leverage latent temporal structure like regime transitions and repeated motifs that existing approaches ignore.

Method: Uses BIC-based statistical divergence to detect change points at multiple temporal resolutions, then summarizes segments using simple functions (mean) or generative models (GMMs). Operates as model-agnostic preprocessor.

Result: Achieves 85-90% of full-model performance with dramatic computational cost reduction. Outperforms uniform and clustering-based compression baselines in robustness under noise and discriminative structure preservation.
...
### [130] [MODE: Learning compositional representations of complex systems with Mixtures Of Dynamical Experts](https://arxiv.org/abs/2510.09594)
*Nathan Quiblier,Roy Friedman,Matthew Ricci*

Main category: cs.LG

TL;DR: MODE (Mixture Of Dynamical Experts) is a graphical modeling framework that decomposes complex biological dynamics into interpretable components, enabling unsupervised discovery of behavioral regimes and accurate forecasting across noisy transitions between different dynamical systems.


<details>
  <summary>Details</summary>
Motivation: Life science dynamical systems often have overlapping behavioral regimes with noisy transitions between them, which challenges traditional flow-based models that assume smooth dynamics. This is particularly problematic in cellular biology where subpopulations shift between cycling, equilibrium, and developmental branching dynamics.

Method: MODE uses a neural gating mechanism within a graphical modeling framework to decompose complex dynamics into sparse, interpretable components. The framework allows agents to jump between different governing laws, making it especially suited for handling noisy transitions between behavioral regimes.

Result: MODE successfully performed unsupervised classification on synthetic dynamical data, including in noisy, few-sample settings. It excelled at forecasting challenging biological processes like cycling and branching dynamics, and on human single-cell RNA sequencing data, it distinguished proliferation from differentiation dynamics and predicted when cells commit to their ultimate fate.

Conclusion: MODE provides an effective framework for analyzing complex biological systems with overlapping behavioral regimes and noisy transitions, addressing key challenges in computational biology for regime discovery and fate prediction in cellular dynamics.
...
### [131] [BaNEL: Exploration Posteriors for Generative Modeling Using Only Negative Rewards](https://arxiv.org/abs/2510.09596)
*Sangyun Lee,Brandon Amos,Giulia Fanti*

Main category: cs.LG

TL;DR: BaNEL is a novel algorithm that post-trains generative models using only failed attempts, minimizing reward evaluations while improving performance in sparse-reward scenarios.
...
Method: BaNEL casts learning from failures as an in-loop generative modeling problem, using this model to identify failure patterns and steer generation away from them while minimizing reward evaluations.
...
Conclusion: The approach demonstrates that learning from failures alone can significantly enhance generative model performance in challenging sparse-reward environments.
